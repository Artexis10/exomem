...
| `advisory.best_cosine` | The advisory's cosine sweep. `texts`/`chars` say what was encoded; `texts=0` with a `vectors` count is a surface that reuses published vectors and encodes nothing: the deferred write advisory, and the recall pack's proximity pass, which adds `pages` for how many packed pages had exact stored rows. |
