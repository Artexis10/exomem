...
idempotency runtime state. An install upgraded from 0.46.0 or earlier can already
have a runtime directory whose inherited ACL does not meet that contract. Exomem
does not silently rewrite it: the service and `exomem doctor` fail closed and print
the exact offending path plus a one-line `icacls.exe` repair for the current runtime
identity.

Stop the process that owns the reported runtime, open an elevated PowerShell as
that intended owner, copy the complete command from the error, run it, then restart
the process. The command resets and protects only the named entry; it is not
recursive. Do not delete the idempotency database to bypass validation. A command
generated by a normal user is not a repair for a LocalSystem-owned path: run the
diagnostic under the identity that owns the runtime.

The trustee set is exact. LocalSystem state permits only LocalSystem and
Administrators; normal-user state permits that user's SID plus LocalSystem and
Administrators. One `EXOMEM_WRITER_LEASE_STATE_DIR` therefore cannot serve both an
NSSM LocalSystem service and a direct user process. More importantly, that directory
also anchors the process-safe mutation lock. Do not point the two identities at
separate state roots and then mutate the same vault concurrently. Route ordinary
mutations through the service. Before direct CLI maintenance such as `exomem index`,
stop the service, use a user-private state root, finish the command, and restart the
service. `exomem mode` only updates machine configuration and does not enter the
vault mutation path.
...
On Windows this runtime state is also principal-private. A LocalSystem service and
a normal-user CLI cannot share one exact DACL or DPAPI receipt envelope. Keep direct
user maintenance quiescent with the service stopped; separate state directories are
an ownership boundary, not permission to create two concurrent mutation locks for
one vault.
