...
The immutable provisioner image carries the exact Alembic configuration and revision tree at a fixed recursively verified root-owned, non-writable path and exposes environment-only bootstrap, migration, and exact-head validation commands. Fresh installation is a split-authority operation over direct or explicitly session-stable PostgreSQL endpoints: a one-shot operator Job briefly receives an admin URL, holds one bounded database/schema advisory lock while it creates or validates the least-privilege runtime role and owned schema, and retains a second unpredictable advisory challenge. The separately authenticated runtime connection must observe that challenge already held in the same database lock domain before it migrates and proves final runtime access; known transaction-pooling shapes are rejected. Incoming and outgoing runtime-role membership both fail closed. The admin Secret and Job are always deleted after success, failure, timeout, or interruption, and the provider credential is rotated or revoked; a private attempt/version/time-bound receipt and persistent prior-attempt state prevent retry bypass. Neither authority nor receipt enters a stable chart resource or long-lived workload. The stable pre-install hook then migrates/idempotently proves the packaged head using only the runtime URL. A pre-upgrade hook is intentionally validation-only: if the database is not already at the new image head, it blocks rollout. Forward-only revision-advancing upgrades require a later expand/contract maintenance design rather than being implied rollback-safe by Helm.
...
Verified recovery objects use seven-day Object Lock retention and a 30-day normal lifecycle window for the private alpha. User-visible exports retain their existing shorter product TTL and are not silently converted into recovery backups. On account deletion, access and billing stop immediately, but the lifecycle remains `deleting`/`retained` until the last lock expires. The credential-free dispatcher creates no Job for an empty queue and at most one admission-scoped short-lived privileged deletion Job for an atomically bound eligible claim. That Job starts from the tenant's durable recovery and plaintext-delivery rows, initializes effective retention from the durable row, and performs explicit-page-size exact-key version/marker checks with hard page/item/cursor bounds and a lexicographic sibling stop rather than a whole-bucket scan. It deletes every recorded exact version and associated marker without governance bypass and re-reads the ledger after provider absence and wrapped-key erasure before returning final proofs. A crash between provider deletion and key erasure therefore resumes from the ledger. Only then does the control plane emit `deleted`. These clocks and the maximum seven-day delay are disclosed in the deletion UI.
...
The EUR 5 friend price is a subsidized private-alpha price. Paddle's public standard fee is 5% + EUR 0.50 per checkout, so a EUR 5 charge yields at most EUR 4.25 before tax treatment; the real account rate and VAT-inclusive/exclusive choice are recorded before catalog activation. Automatic provisioning stops at six active USER cells and two RECOVERY/restore-candidate cells, with no more than eight potential or attached cell volumes. That preserves eight unused attachments beneath Hetzner's sixteen-volume server ceiling without double-counting recovery cells as both real volumes and synthetic reserve.

Admission is restart- and race-safe. A collector with the signing seed and read-only HCloud token emits a five-minute Ed25519 receipt bound to the kube-system UID, exact foundation server ID/location, separate live USER/RECOVERY namespace counts, and exact-server attachment count. Routine and volume-registration workers receive only its public verifier, an immutable contract, and Kubernetes read access. Each worker reconciles the signed receipt against a fresh strict Namespace/Node/PV/PVC/VolumeAttachment observation, then locks the existing claim/fence chain followed by the singleton PostgreSQL capacity ledger and active reservation rows. Reservations survive pending work, claim loss, process restart, and terminal provision failure; only a final provider-proved DISCARD or DESTROY releases them atomically with operation completion. A later valid receipt may restart its collector sequence because signature, five-minute freshness, cluster/server binding, live reconciliation, and the serialized ledger are the replay controls.

Increasing the cap requires a fresh resource soak and a monthly cost sheet covering server, IPv4, all volumes, B2 retained bytes/operations, Neon/Vercel marginal cost, Paddle fees, and alerting. The owner proof starts with no paid seats; the first friend is invited only after the measured fixed and marginal cost is visible.
