...
The immutable provisioner image carries the exact Alembic configuration and revision tree at a fixed root-owned, non-writable path and exposes environment-only bootstrap, migration, and exact-head validation commands. Fresh installation is a split-authority operation: a one-shot operator Job briefly receives an admin URL, holds one bounded database/schema advisory lock while it creates or validates the least-privilege runtime role and owned schema, migrates through a separately authenticated runtime connection, and proves final runtime access. The admin Secret and Job are always deleted and the provider credential rotated or revoked; neither enters a stable chart resource or long-lived workload. The stable pre-install hook then migrates/idempotently proves the packaged head using only the runtime URL. A pre-upgrade hook is intentionally validation-only: if the database is not already at the new image head, it blocks rollout. Forward-only revision-advancing upgrades require a later expand/contract maintenance design rather than being implied rollback-safe by Helm.
