...
7. Compensation is a new reviewed plan, never rollback. Use `maintain_memory(mode="curation", curation_action="propose-compensation", run_id=...)`, preview it, obtain a fresh exact-plan confirmation, then use `maintain_memory(mode="curation", curation_action="apply-compensation", run_id=..., plan_id=..., expected_plan_fingerprint=..., why=...)`; continue only through the same one-step `resume` route.
