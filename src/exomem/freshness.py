...
# which (vault_root_str, scope) are currently inside the watcher's own
# `seed()` walk -- announced just before that call and cleared in a `finally`
# around it regardless of outcome. See `recall_seed_pending` for why a scope
# that has never gone live still needs this distinction.
_seed_pending: set[tuple[str, str]] = set()
...
def announce_seed_pending(vault_root: Path, scope: str) -> None:
    """Mark one scope's initial `seed()` walk as in flight.

    Called by the file watcher's reconcile thread immediately before it starts
    the boot-time seed walk for `scope` (`seed=True` in `_reconcile_once`),
    and always cleared afterward via `clear_seed_pending`, success or failure.
    """
    with _lock:
        _seed_pending.add(_key(vault_root, scope))


def clear_seed_pending(vault_root: Path, scope: str) -> None:
    """Clear one scope's in-flight seed mark, however the attempt ended."""
    with _lock:
        _seed_pending.discard(_key(vault_root, scope))


def recall_seed_pending(vault_root: Path, scope: str) -> bool:
    """Whether a scope that has never gone live is actively being seeded.

    A caller deciding whether to abstain and WAIT for a cold scope to converge
    (rather than fall back to its own cold walk right now) needs more than
    "not live yet" -- a scope that will never converge without outside help
    (the watcher never started at all, e.g. a missing KB directory; or its one
    boot-time seed attempt raised and nothing else is retrying it before the
    next 300s periodic reconcile) would otherwise read as cold forever, and a
    caller that abstains on cold alone would then abstain forever too instead
    of resolving through the pre-existing fallback path.

    `True` only while the watcher's own `seed()` walk for this exact scope is
    in flight. `False` once the scope is live (moot for this check -- callers
    test `recall_is_live`/`is_live` first) and equally `False` once the seed
    attempt has ended, whether it succeeded or not.
    """
    if not event_indexes_enabled():
        return False
    with _lock:
        return _key(vault_root, scope) in _seed_pending
