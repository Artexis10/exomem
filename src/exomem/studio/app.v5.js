...
import {
  ...
  sectionState,
  visibleItems,
  worklistFiltersVisible,
} from "/studio/assets/model.v2.js";
...
async function loadRelationQueue({focusRef = ""} = {}) {
  byId("workspace-content").hidden = true;
  byId("workspace-empty").hidden = true;
  ...
  try {
    ...
    renderRelationQueue();
    if (focusRef) focusRelationCandidate(focusRef);
  } catch (error) {
    if (error instanceof ApiError && (error.status === 401 || error.code === "REST_DISABLED")) {
      showAuth(error);
      return;
...
function renderRelationQueue() {
  ...
  renderQueueCoverage(model);
  const groups = model.groups;
  status.textContent = relationQueueStatus(model);
  ...
  replaceChildren(
    ...
    groups.length
      ...
      : [relationQueueStateNote(model)],
  );
}

function relationQueueStateNote(model) {
  if (model.state === "warming") {
    return element("p", "The relation graph is warming. Retry with Refresh worklist; no review decision was changed.", "section-note warming");
  }
  if (model.state === "pending") {
    return element("p", "Relation review is pending graph recovery. Retry with Refresh worklist; no review decision was changed.", "section-note pending");
  }
  if (model.state === "unavailable") {
    return element("p", "Graph-backed relation review is unavailable. Retry with Refresh worklist; no review decision was changed.", "section-note unavailable");
  }
  return element("p", "No relation candidates await review in this bounded server view. Accepted edges leave the queue automatically.", "section-note empty");
}

function renderQueueCoverage(model) {
  const node = byId("coverage");
...
function renderRelationCandidate(item, group) {
  ...
  summary.append(element("span", `source: ${item.source_path}`, "path"));
  li.append(summary);
  const actions = element("div", "", "actions");
  const accept = element("button", "Accept");
  accept.type = "button";
  accept.dataset.relationRef = item.ref;
  accept.dataset.relationAction = "accept";
  accept.setAttribute("aria-label", `Accept ${item.relation_type || "relates_to"} → ${destination} from ${item.source_path}`);
  accept.addEventListener("click", () => openRelationAccept(item, group));
  const dismiss = element("button", "Dismiss");
  dismiss.type = "button";
  dismiss.dataset.relationRef = item.ref;
  dismiss.dataset.relationAction = "dismiss";
  dismiss.setAttribute("aria-label", `Dismiss ${item.relation_type || "relates_to"} → ${destination} from ${item.source_path}`);
  dismiss.addEventListener("click", () => openRelationTriage(item, "dismiss"));
  const snooze = element("button", "Snooze");
  snooze.type = "button";
  snooze.dataset.relationRef = item.ref;
  snooze.dataset.relationAction = "snooze";
  snooze.setAttribute("aria-label", `Snooze ${item.relation_type || "relates_to"} → ${destination} from ${item.source_path}`);
...
function openRelationTriage(item, action) {
  ...
  const whyLabel = element("label", "Audit reason");
  const why = document.createElement("textarea");
  why.name = "why";
  why.rows = 3;
  why.required = true;
  why.value = action === "dismiss" ? "Dismissed reviewed relation" : "Snoozed reviewed relation";
  whyLabel.append(why);
  fields.push(whyLabel);
  replaceChildren(byId("dialog-fields"), fields);
  byId("dialog-error").hidden = true;
...
async function submitRelationAccept() {
  ...
  await guardedWrite(
    ...
    {
      precheck: false,
      onDone: () => loadRelationQueue({focusRef: item.ref}),
      onDrift: async () => {},
      driftCodes: ["REVIEW_ITEM_CHANGED", "REVIEW_REFRESH_REQUIRED", "STALE_EDIT"],
      driftMessage: "The relation candidate or source changed. Nothing was written; refresh required before another decision. The visible candidate is unchanged.",
    },
  );
}
...
async function submitRelationTriage() {
  ...
  const why = String(data.get("why") || "").trim();
  if (!why) {
    showError(byId("dialog-error"), new ApiError("An audit reason is required to triage a relation."));
    return;
  }
  await guardedWrite(
...
function focusRelationCandidate(ref) {
  const acceptButtons = [...document.querySelectorAll('[data-relation-action="accept"]')];
  const target = acceptButtons.find((node) => node.dataset.relationRef === ref) || acceptButtons[0];
  (target || byId("refresh"))?.focus();
}
...
function wireEvents() {
  ...
  dialog.addEventListener("close", () => {
    if (route.mode === "relation-queue") focusRelationCandidate(restoreRef);
    else focusItem(restoreRef);
  });
  for (const tab of document.querySelectorAll("[data-view]")) {
    tab.addEventListener("click", async () => {
