...
`KB_MCP_DISABLE_USAGE_BOOST`, the suite's `KB_MCP_DISABLE_RELEVANCE_CHECK`
...
def access_events(
    ...
    *,
    ...
    (`KB_MCP_DISABLE_RELEVANCE_CHECK`) or all three logs empty — so callers
...
def boost_disabled() -> bool:
    ...
    return bool(
        os.environ.get("KB_MCP_DISABLE_USAGE_BOOST")
        or os.environ.get("KB_MCP_DISABLE_RELEVANCE_CHECK")
    )
...
# every KB_MCP_USAGE_REFRESH_S seconds (default 300) and any change rebuilds.
...
def _refresh_seconds() -> float:
    raw = os.environ.get("KB_MCP_USAGE_REFRESH_S")
    if raw is None or not raw.strip():
        return _DEFAULT_REFRESH_S
    try:
        return max(0.0, float(raw))
    except ValueError:
        log.warning("KB_MCP_USAGE_REFRESH_S=%r is not a number; using default", raw)
        return _DEFAULT_REFRESH_S
