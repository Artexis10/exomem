...
CONTRADICTION_FLOOR = 0.82  # default lower edge of the contradiction band [floor, dup_threshold); override via KB_MCP_CONTRADICTION_FLOOR
...
def _contradiction_floor() -> float:
    """CONTRADICTION_FLOOR, overridable at runtime via KB_MCP_CONTRADICTION_FLOOR.
    ...
    """
    raw = os.environ.get("KB_MCP_CONTRADICTION_FLOOR")
    if raw is None:
    ...
    try:
        ...
        log.warning(
            "invalid KB_MCP_CONTRADICTION_FLOOR=%r; using %s", raw, CONTRADICTION_FLOOR
        )
        return CONTRADICTION_FLOOR
...
def detect_duplicates(
    ...
    *,
    ...
    `threshold` (default resolved from `KB_MCP_DUP_THRESHOLD`, else
...
def detect_contradictions(
    ...
    *,
    ...
    `floor` is resolved from `KB_MCP_CONTRADICTION_FLOOR`, the `ceiling` from
    `KB_MCP_DUP_THRESHOLD`; an inverted band (floor >= ceiling) is logged and
    ...
    if floor >= ceiling:
        log.warning(
            "KB_MCP_CONTRADICTION_FLOOR (%s) >= dup ceiling (%s); "
