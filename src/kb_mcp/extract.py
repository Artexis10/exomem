...
def _env_flag(name: str) -> bool:
    ...
    presence check would read `KB_MCP_DIARIZE=0` as opted IN.
    """
    return os.environ.get(name, "").strip().lower() not in _FALSY_ENV


def _diarize_enabled() -> bool:
    """True only when KB_MCP_DIARIZE is set truthy. OFF by default — diarization never
    changes existing extraction unless explicitly opted in."""
    return _env_flag("KB_MCP_DIARIZE")
...
def _run_diarization(path: Path) -> list[tuple[float, float, str]] | None:
    ...
    if py is None:
        log.warning(
            "KB_MCP_DIARIZE is set but the diarizer sidecar venv is not provisioned "
...
def _vision_caption_enabled() -> bool:
    ...
    return _env_flag("KB_MCP_VISION_CAPTION")
