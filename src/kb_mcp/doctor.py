...
def _resolve_vault(vault: str | None) -> tuple[Path | None, DoctorCheck]:
    raw = vault or os.environ.get("KB_MCP_VAULT_PATH")
    if not raw:
        return None, _check(
            "vault.path",
            "fail",
            "No vault path supplied and KB_MCP_VAULT_PATH is unset.",
            "Set KB_MCP_VAULT_PATH to the vault root or pass --vault. Then run "
            "`uv run python -m kb_mcp init --vault <path>` if the vault is new.",
        )
    ...
    if not skill.exists():
        return path, _check(
            ...
            "`uv run python -m kb_mcp init --vault <path>`.",
        )
...
def _check_package_import() -> DoctorCheck:
    try:
        importlib.import_module("kb_mcp")
    except Exception as e:  # noqa: BLE001 - this is a diagnostic boundary
        return _check(
            "package.import",
            "fail",
            f"Could not import kb_mcp: {e}",
            "Run `uv sync` from the repo root, then retry with `uv run python -m kb_mcp doctor`.",
        )
    return _check("package.import", "pass", "kb_mcp imports successfully.")
    ...
    for id_, path, label in required:
        if path.exists():
            ...
            checks.append(_check(
                ...
                status,
                f"{label} is missing.",
                "Run `uv run python -m kb_mcp init --vault <path>` for a new vault, or "
                "restore the missing scaffold file from src/kb_mcp/_scaffold/.",
            ))
    return checks
...
def _check_embeddings_disabled() -> DoctorCheck:
    if os.environ.get("KB_MCP_DISABLE_EMBEDDINGS"):
        return _check(
            "embeddings.enabled",
            "fail",
            "KB_MCP_DISABLE_EMBEDDINGS is set, so hybrid/vector search is disabled.",
            "Unset KB_MCP_DISABLE_EMBEDDINGS for hybrid search after installing "
            "`uv sync --extra embeddings`.",
        )
    return _check("embeddings.enabled", "pass", "KB_MCP_DISABLE_EMBEDDINGS is not set.")
...
def _check_tesseract() -> DoctorCheck:
    configured = os.environ.get("KB_MCP_TESSERACT_CMD")
...
def _check_remote_env() -> list[DoctorCheck]:
    required = {
        "KB_MCP_BASE_URL": "Set the public HTTPS base URL, e.g. https://kb.example.com.",
        ...
        "KB_MCP_GITHUB_USERNAME": "Set the single GitHub login allowed to authenticate.",
        "KB_MCP_JWT_SIGNING_KEY": "Generate a stable signing key, e.g. python -c \"import secrets; print(secrets.token_urlsafe(48))\".",
    }
    checks: list[DoctorCheck] = []
    ...
    checks.append(_check("env.KB_MCP_HOST", "pass", f"KB_MCP_HOST resolves to {host}."))
    if os.environ.get("KB_MCP_REST_API_KEY"):
        checks.append(_check("env.KB_MCP_REST_API_KEY", "pass", "REST API key is set."))
    else:
        checks.append(_check(
            "env.KB_MCP_REST_API_KEY",
            "warn",
            "KB_MCP_REST_API_KEY is unset; /api/* stays disabled.",
            "Run `uv run --no-sync python scripts/set-rest-key.py` if you want REST access.",
        ))
    if os.environ.get("KB_MCP_UPLOAD_TOKEN"):
        checks.append(_check("env.KB_MCP_UPLOAD_TOKEN", "pass", "Upload token is set."))
    else:
        checks.append(_check(
            "env.KB_MCP_UPLOAD_TOKEN",
            "warn",
            "KB_MCP_UPLOAD_TOKEN is unset; upload/download token minting stays disabled.",
