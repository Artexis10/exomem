...
- **New scope not in the project list.** If Hugo names a scope that isn't a known project key, surface this — don't silently invent. Either it maps to an existing key, or the scope list needs to be widened (a small schema update).
