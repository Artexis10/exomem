...
| **preserve** | Capture a binary / factual artifact for an incident scope (tiny files only — larger binaries go via the out-of-band `/upload` endpoint, never base64) | `Evidence/<scope>/` |
...
- **Binaries never travel through the model as base64.** Encoding a file (image, PDF, screenshot, any binary) into a tool argument is billed as *output tokens* — a multi-MB file is millions of them, and a chat-uploaded image can't be reproduced faithfully anyway. Instead: **transcribe** what you can see into the note/evidence *text* (that's what's queryable), and deliver the *original file* out-of-band. On **claude.ai web** this is hands-off — call **`mint_upload_token`** for a short-lived `{token, upload_url}`, then have the code sandbox multipart-`curl` the user's **attached** files to `upload_url` (no base64, no pasted secret; see `references/operations.md` § preserve). Fallbacks: hand Hugo the prefilled `https://kb.substratesystems.io/upload?scope=…&category=…` link, or a desk-side Obsidian Sync drop into `Evidence/<scope>/<category>/`. `preserve(content_base64=…)` is for genuinely small artifacts only; the text-write tools hard-reject base64 blobs (`BINARY_BLOB_REJECTED`). Full workflow: `references/operations.md` § preserve.
