...
| **preserve** | Capture a **text** factual artifact for an incident scope. Binaries (PDF / image / any file) go out-of-band via `/upload` (`mint_upload_token` on web), not this tool — pass the artifact's extracted/OCR text as `/upload`'s `text` field so the opaque binary gets a **findable** text companion | `Evidence/<scope>/` |
...
- **Binaries go out-of-band — never inline through a tool argument.** **Transcribe** what's relevant into the note/evidence *text* (that's the queryable part), and deliver the *original file* separately. On **claude.ai web** this is hands-off — call **`mint_upload_token`** for a short-lived `{token, upload_url}`, then have the code sandbox multipart-`curl` the user's **attached** files to `upload_url` (no inline bytes, no pasted secret; see `references/operations.md` § preserve). **Make the binary searchable:** in the same sandbox, extract the file's text — OCR an image/scan, read a PDF, transcribe audio/video — and send it as the `text` form field on `/upload`. It lands in an embedded sidecar so the otherwise-opaque binary is findable by its content. Fallbacks: hand Hugo the prefilled `https://kb.substratesystems.io/upload?scope=…&category=…` link, or a desk-side Obsidian Sync drop into `Evidence/<scope>/<category>/`. `preserve` and the other write tools take **text only**; they reject inline byte blobs (`BINARY_BLOB_REJECTED`). The bytes never pass through the model — full workflow: `references/operations.md` § preserve.
- **Pull a vault file back out — the `/download` reverse channel.** To work *on* a stored file in the sandbox (analyze a dataset, re-read an evidence scan), call **`mint_download_token`** for a short-lived `{token, download_url}`, then GET `download_url?path=<vault-relative path>` with `Authorization: Bearer <token>`. Read-only, download-scoped (can't write), path confined to the vault root. The bytes stream out-of-band — never back through the model. See `references/operations.md` § download.
