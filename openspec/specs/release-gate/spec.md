...
#### Scenario: A restricted move judged again without a skipped linker

- **WHEN** a restricted writer's move skips a withheld linker whose rewrite its contract
  would refuse, and is judged again without it
- **THEN** the answer is byte-identical to the twin without that page, and the extra
  judgement's time is an accepted low-severity residual of the same class as the recall
  limit, not a disclosure the answer carries
...
For a writer other than the owner under a governed policy, a write door SHALL decide its target page before resolving, reading or changing it. `edit_memory`, `observe_memory` by path or memory reference, `replace_memory`, and `manage_memory_file` `append`, `move` source, `delete` of a file, and `reclassify` SHALL answer a target the writer may not see exactly as a missing target, and SHALL leave it unchanged. A folder delete by such a writer SHALL be refused with one `AUDIENCE_RESTRICTED` answer whatever a folder it may see holds, and a declared recursive delete SHALL be refused before anything is read; a folder holding only pages withheld from it SHALL be answered as a missing path, as its listing is. A creation door whose destination a withheld page occupies SHALL answer with its ordinary occupied-destination refusal, naming no path the writer did not supply, and SHALL leave that page unchanged, including under `overwrite`. A move that updates wikilinks SHALL rewrite every linking page, and SHALL report counts, paths and contract results for the linking pages the writer may see, with no index report and no derived-graph outcome, always as the committed terminal, so its answer in every response detail, on that move and every later one, is the same whether or not a withheld page linked the moved one. A withheld linking page the move cannot rewrite cleanly, because it sits in an append-only tree or because its contract would refuse the rewritten page, SHALL be left unchanged and the move SHALL commit, so that page's link dangles exactly as it would with `update_wikilinks=false`; the owner's audit reports it as a `broken_wikilink` or `forward_reference`. A withheld linking page the move can rewrite cleanly SHALL be rewritten. The activity log line such a writer's move or file delete appends SHALL carry only the figures that writer may see. A file delete by such a writer SHALL count, list and log only inbound links from pages it may see; a link from a page withheld from it SHALL be orphaned as `force_orphan` would orphan it, and the owner's audit reports it. A reclassification proposal by such a writer SHALL count as references only inbound links from pages it may see. A page withheld from the mover whose bytes the move does not rewrite SHALL NOT be asserted against the move, even when the move changes how that page's own links resolve; it stays in the closure the move publishes, and its standing is the owner's to review. These rulings SHALL apply under a non-empty policy only; on a vault with no policy every caller's writes SHALL be answered as the owner's are. The owner's writes SHALL be unchanged.
...
#### Scenario: A withheld page the move does not rewrite

- **WHEN** a restricted writer's move, with or without updating wikilinks, changes how a non-compliant withheld page's own link resolves, and the move does not rewrite that page
- **THEN** the move's answer, in every response detail, matches the twin without that page, and that page is unchanged

#### Scenario: A withheld page links a page a restricted writer deletes

- **WHEN** a restricted writer deletes a page that a withheld page links, with or without `force_orphan`
- **THEN** the inbound refusal, the delete's answer and the activity log line match the twin without that page, in every response detail, and the owner's audit reports the withheld page's orphaned link

#### Scenario: A withheld linker the move cannot rewrite cleanly

- **WHEN** a restricted writer moves a page, with `update_wikilinks=true`, that a withheld page in an append-only tree links, or that a withheld page links whose contract would refuse the rewrite
- **THEN** the move commits with an answer, in every response detail, matching the twin without that page; that page is unchanged, and the owner's audit reports its dangling link
...
A count or rank a restricted caller receives SHALL be computed over the entries it receives, and retrieval diagnostics computed before release decisions SHALL NOT be returned to it. Under a governed policy such a caller's recall SHALL run without the graph lane and graph enrichment, decided before the shared recall cache is consulted; its link suggestions SHALL rank over the pages it may see, without the graph lane and without whole-corpus rank numbers, from one recall of the fixed over-fetch pool that recall itself uses, so that a query withheld pages dominate returns fewer suggestions rather than taking more work. A whole-vault aggregate that a restricted view cannot recompute SHALL be served to the owner only under a governed policy, and SHALL answer other audiences with `available: false` and `reason: "audience_restricted"`, decided before anything is read.
