# context-activation-continuity Specification

## Purpose
Let a conversation keep the context it already earned without the server holding any session state: `activate_context` returns a client-carried `continuity` token naming the anchors a packet resolved, and a later turn that presents it lets those anchors resolve on a single contact kind, never on the token alone. An `anchor` override lets the agent settle an ambiguous turn by naming its choice. A token from another index, an undecodable token or an oversized one is reported as `stale` and changes nothing.
