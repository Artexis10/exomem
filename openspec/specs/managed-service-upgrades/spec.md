...
Admission during that wait SHALL remain bounded and explicit, with no request held without a bound, and the operator tooling's deadline for polling a transition SHALL cover the standby warm, the cutover and the cold-start budgets, so it does not report failure while the supervisor is still legitimately waiting. The handoff record SHALL report the time from the previous worker's stop to the replacement's readiness, on a failed handoff as well as a successful one, and SHALL report what the replacement last reported waiting on as `replacement_waiting`, distinct from the `waiting` component of a discarded standby, so a handoff that discarded a candidate and then failed to start its replacement carries both. None of these records SHALL affect any budget, wait or outcome.
