# cloud-node-pool Specification

## Purpose
Grow and shrink the Exomem Cloud K3s fleet through infrastructure as code. One
Terraform map entry declares an agent server. An idempotent Ansible join gives
it the existing node's hardening and a dedicated, CA-pinned agent token, and an
ordered removal never over-commits admission or strands a live node. Capacity
follows from the cluster's own observation, with no controller change.
