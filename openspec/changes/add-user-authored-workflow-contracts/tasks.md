...
- [ ] 2.2 Wire every workflow-contract operation through the registered `schema_memory` command so MCP, REST, CLI, envelopes, dry-run semantics, stale-write errors, and audit receipts remain one implementation; add selector-level lease/receipt classification while keeping the command statically mixed/write-capable.
...
- [ ] 3.5 Prove parity, compact-budget bounds, hand-authored scaffold genericity, portable-offline usefulness, and full/compact capability separation with focused tests.
...
- [ ] 4.3 Replace privileged OpenSpec/repository wording across canonical runtime descriptions, validators, bootstrap, scaffold, and knowledge packs; broaden Planning `execution.kind` to the exact open key syntax while preserving every existing enum value as valid opaque data.
...
- [ ] 5.1 Document standalone and companion workflows, scope resolution, safe editing in Markdown/Obsidian, ambiguity and invalid-file recovery, reviewed ephemeral overrides, and the Records feedback loop.
- [ ] 5.2 Document the representation-compatible but behaviour-explicit migration and rollback path, including the migration-required refusal, current-session standalone selection, durable saved choice, and unchanged Planning/Records files.
- [ ] 5.3 Document how future integrations and the hosted portal consume the stable resolver/save surface without making external tools or a portal part of this delivery.

## 6. Independent acceptance and closure

- [ ] 6.1 Run focused workflow-contract, command-surface, bootstrap, Planning, Records, portability, security, and documentation tests plus lint/type checks for touched modules.
