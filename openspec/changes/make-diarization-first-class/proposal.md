...
3. **`EXOMEM_DIARIZE=0` enables diarization.** The gate is a presence check
   (`bool(os.environ.get(...))`), so `0`/`false` opt IN. Same bug class in `EXOMEM_VISION_CAPTION`.
...
- `backfill-media --rediarize`: re-extract audio/video sidecars whose engine is a completed ASR
  ...
  message when `EXOMEM_DIARIZE` is off); circuit-breaks when diarization soft-fails mid-pass
...
- Truthy env parse for opt-in flags: `""`/`0`/`false`/`no`/`off` (case-insensitive) now read as
  OFF for `EXOMEM_DIARIZE` and `EXOMEM_VISION_CAPTION` (strictly tightens default-off).
...
- Code: `src/exomem/backfill.py` (`_extracted_engine`, `_needs_rediarize`, `rediarize=` param,
  `BackfillStats.rediarized`, `speakers=` fix), `src/exomem/extract.py` (`_env_flag`,
  `log_diarization_readiness`, WARNING on unprovisioned sidecar), `src/exomem/media_worker.py`
  (readiness call in `start()`), `src/exomem/__main__.py` (`--rediarize` flag).
