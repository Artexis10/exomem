...
- [x] 1.1 Red test `tests/test_governance_principal.py`: per-surface resolver
...
- [x] 1.2 Implement `RequestPrincipal` + contextvar + `request_scope()` (clone of
...
- [x] 2.1 Red test `tests/test_governance_egress.py::test_op_find_annotates_and_filters`:
      withheld hit → notice; permitted hit → decision; pack excludes withheld;
      graph seed / relation_match naming withheld path scrubbed.
- [x] 2.2 Implement `project(payload, level)` per-level allow-list as the only
...
- [x] 2.3 Insert `annotate_hits` at `commands.py:901→902` (after `find()`, before
      `assemble_pack`); request-deterministic backfill from an over-fetch pool;
      L0 silent at pool exhaustion.

## 3. Cache invariant

- [x] 3.1 Red test `test_governance_egress.py::test_find_hot_cache_stays_principal_free`
...
- [x] 4.1 Red tests `test_get_respects_decision_levels`,
      `test_graph_context_guard_seed_governed`, pack-decision test.
- [x] 4.2 `annotate_page` in `op_get` (`commands.py:1838+`); `guard_seed` in
      `op_graph_context`/`epistemic_graph`; pack elements carry decisions.
- [x] 4.3 Add `purpose` param to `op_find`/`op_ask_memory`/`op_read_memory`/
...
- [x] 5.1 Red test `tests/test_governance_tokens.py`: mint/verify/expire/
      single-use-redeem/sweep; content-fingerprint binding; drift refuses.
- [x] 5.2 Implement `wh1.` token format, per-machine sidecar HMAC key,
      `BEGIN IMMEDIATE` consume-once; withheld notices embed tokens.

## 6. Terminal scrubber + postfilter (scrubber.py, egress.postfilter)

- [x] 6.1 Red test `tests/test_governance_postfilter.py`: credential patterns
...
- [x] 6.2 Implement `scrubber.py` (always-on) + `egress.postfilter`; call in
...
- [x] 6.3 Red tests `test_release_gate_runs_on_rest_surface`,
      `test_release_gate_runs_on_cli_surface`,
      `test_retrieve_inject_hook_respects_release` (surface parity).

## 7. Transfer / media gating

- [x] 7.1 Red tests `test_transfer_download_denies_below_l6`,
      `test_read_media_frames_gated`.
- [x] 7.2 `op_transfer_artifact` download-target selection and
      `op_get_video_frames` consult the release decision before minting/returning.

## 8. Gates

- [x] 8.1 `PYTHONPATH=src EXOMEM_DISABLE_EMBEDDINGS=1 uv run python -m pytest -q
...
- [x] 8.2 New `tests/test_governance_overhead.py`: `op_find` empty-policy delta
      < 5 ms median over `gen_dense_vault` 2k; scrubber < 2 ms per 100 KB.
- [x] 8.3 `uv run python -m pytest tests/test_latency_gate.py -q` +
      `tests/test_retrieval_golden.py` (embeddings lane) green — thresholds and
      goldens unchanged.
- [x] 8.4 `uvx ruff check` clean; `openspec validate add-release-gate --strict`
      green.
