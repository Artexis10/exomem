...
- [x] 1.1 Add snapshot-equivalence tests with monkeypatched walk counters proving a single hybrid
      ...
      stat-walk.
- [x] 1.2 Add staleness tests proving a write to a markdown file in scope is seen by the next `find`
      call for both BM25 (KB and vault scope) and the wikilink resolver.
- [x] 1.3 Add freshness-key tests for deletes, renames (mtime preserved), and backdated replacements
      ...
      `(count, max_mtime)` key.
- [x] 1.4 Add `ParsedPage` derived-text tests proving `body_norm`/`title_norm`/`stem_set` are
...
- [x] 1.5 Add a combined-multiplier equivalence-grid test comparing the new single-pass output
...
- [x] 1.6 Add `InboundLinkIndex` tests proving output and ordering are identical to the current
...
- [x] 1.7 Add a `find(pack=true)` regression test proving neighborhood assembly (including ordering)
      is unchanged when backed by the cached inbound-link index.
- [x] 1.8 Add warm-up tests proving `warm_caches` populates the BM25 (KB and vault scope), resolver,
...
- [x] 2.1 Add a `FreshnessSnapshot` type with lazy, memoized `.kb()` and `.vault()` accessors that
...
- [x] 2.2 Thread an optional `freshness` keyword through `bm25.BM25Index.search`/`bm25.search` so a
...
- [x] 2.3 Thread the same optional `freshness` keyword through `_get_query_resolver`.
- [x] 2.4 Upgrade the BM25 cache comparison (`current_max > cached[0]`) and the resolver's
      `(count, latest)` cache key to `(count, max_mtime_ns, digest-of-sorted-rel-paths)`.
- [x] 2.5 Create exactly one `FreshnessSnapshot` per `find()` call and pass it to every `bm25.search`
...
- [x] 3.1 Add `body_norm`, `title_norm`, and `stem_set` as `functools.cached_property` members on
      `ParsedPage`, matching `_make_excerpt`'s current normalization exactly and lazy-importing
      `bm25.tokenize` for `stem_set`.
- [x] 3.2 Update `_make_excerpt`, `_keyword_match_paths` (and the keyword-mode all-tokens-present
...
- [x] 3.3 Document on `ParsedPage`/`FrontmatterCache` that derived-text invalidation relies on
...
- [x] 4.1 Add `_apply_post_rrf_multipliers` computing the combined type/status/temporal multiplier
...
- [x] 4.2 Give it a fused-score mode (operating on `(path, score)` RRF pairs) and a rerank-score mode
...
- [x] 4.3 Replace the three sequential fused-score calls with the new combined pass, and replace the
      post-rerank re-apply block with the rerank-mode call.
- [x] 4.4 Add a per-request `dict[str, ParsedPage | None]` page memo shared by the multiplier pass,
      the fused-candidate-to-`Hit` resolution loop, and auto-widen's strong/weak partition.
- [x] 4.5 Remove `_apply_type_boost`, `_apply_status_demotion`, and `_apply_temporal_boost` once the
      combined pass covers their behavior, after confirming no other call site depends on them.

## 5. InboundLinkIndex

- [x] 5.1 Add a module-level, process-cached inbound-link index in `vault.py`, built in one read pass
...
- [x] 5.2 Freshness-key the index by a digest of sorted `(rel_path, mtime_ns)` pairs, not a
      `(count, max_mtime)` pair.
- [x] 5.3 Reimplement `find_inbound_wikilinks` to look up the cached index, preserving its exact
      signature, self-exclusion, basename-uniqueness gate, and seq-merged output ordering.
- [x] 5.4 Wire a reset hook into `find.clear_cache()` or add `vault.clear_link_index()`, and use it
      wherever tests already rely on `find.clear_cache()` to reset freshness-keyed state.
- [x] 5.5 Verify `move_file`/`delete_file`'s existing inbound-link safety checks observe the new
      cache correctly after a rename.

## 6. Startup Warm-Up

- [x] 6.1 Add `bm25.warm(vault_root, scope)`, a thin explicit wrapper that primes the module-level
      `BM25Index` cache for the given scope.
- [x] 6.2 Add `src/kb_mcp/warmup.py` with `warm_caches(vault_root)`: walk the KB through `find._CACHE`,
      ...
      enabled.
- [x] 6.3 Call `warm_caches(vault_root)` synchronously in `build_server`, immediately after the
      existing model preload block, gated by `KB_MCP_DISABLE_WARMUP`.
- [x] 6.4 Give each warm-up stage its own soft-fail `try/except` with a warning log and a duration
      log, matching the existing preload block's pattern.

## 7. Validation

- [x] 7.1 Run targeted tests for `find`, hybrid search, context packs, inbound links, and warm-up.
- [x] 7.2 Run the full suite: `KB_MCP_DISABLE_EMBEDDINGS=1 uv run python -m pytest -q`.
- [x] 7.3 Run `uv run ruff check`.
- [x] 7.4 Confirm no `find`/command-registry parameter, MCP schema, or default `find` behavior
