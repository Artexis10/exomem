...
- [x] 9.3 Run strict OpenSpec validation, focused Records suites, repository full tests with embeddings disabled, Ruff, mypy, generated-surface checks, scaffold leak checks, and proportional latency/scale gates; record exact results.
- [x] 9.4 Run an independent architecture/code/security review focused on Planning overlap, duplicate substrate, exact-span writes, concurrency, manual edits, identity/schema evolution, tracker compatibility, governance/aggregate leakage, recall pollution, source-of-truth promotion, template coupling, hidden DB dependence, stale/ambiguous updates, X3 leakage, migration claims, and Windows publication.
- [x] 9.5 Correct every material reviewer finding and rerun the affected plus full verification rather than merely documenting failures.
- [x] 9.6 Run an independent end-to-end verifier through generated public surfaces and real fixture files, then record X3, vehicle, dataset, Planning-link, governance, retrieval, and manual-edit evidence.
- [x] 9.7 Capture the durable architecture decision, delivered behavior, compatibility boundary, verification evidence, remaining deferred work, and any discovered failure modes back into Exomem with links to the earlier Planning and Records notes.
- [x] 9.8 Commit only intended scope, integrate current `origin/main` in the feature checkout, rerun affected/full verification, push the feature branch, and open a ready pull request without merging it.

### Verification evidence (2026-08-09)

- Strict contracts: `openspec validate add-first-class-records --strict` passed; `openspec validate --specs --strict` passed all 31 main specifications.
- Focused integration after final checkpoint/publication and Windows namespace-pinning corrections: 322 passed with 7 Linux-only platform skips. Native Windows verification passed all 8 selected guarded-read and transactional-publication tests, including the parent-substitution regression.
- Repository lean suite after integrating `origin/main` at `9665d31`, with embeddings and media extraction disabled: 8,158 passed, 131 environment/platform skips, zero failures in 827.04 seconds. All 26 TUI snapshots passed. The suite retains known repository warning output, including optional-dependency deprecations and the existing mocked media-supervisor thread warning.
- Post-PR CI correction under the exact Python 3.11 lean shape: 8,009 passed, 132 expected environment/platform skips, and zero failures in 654.28 seconds. The recreated 3.11 environment omitted the optional TUI extra; the separate unchanged TUI gate remains 159 passed with all 26 snapshots. A Python 3.11-only pytest crash was traced to a Windows-dispatch test mutating process-global `os.name`; the test now uses a narrow module dispatch seam and passes both alone and in full-suite order.
- Generated contracts, bootstrap, packs, scaffold, fixture fidelity, and Records product E2E: 114 passed. TUI: 159 passed with 26 snapshots.
- Packaged product after the final integration: `scripts/e2e_product_loop.py` passed wheel install, governed stdio lifecycle/restart, authenticated HTTP/Studio, and two-replica writer coordination in 16.1 seconds. Wheel-only time-to-value passed in 3.6 seconds, including `exomem demo --json` and unattended setup; the standalone demo passed in 0.528 seconds.
- Static/public artifacts: both required Ruff gates passed; the CI-shaped mypy gate passed 7 source modules; `git diff --check`, capability generation, lock validation, the 2,304-file/2,320-text-payload public repository privacy scan, both distributions, ten skill archives, and the 15-file generated-artifact scan passed. The post-merge generated registry contains 27 tools with matching schema and pending deployment-contract hash `e989c9a5ab165fc250aa03947411d4f5c6b21e3db199bf355fe29fe49f3a9c4b`; 201 conflict-sensitive contract and product tests passed.
- Scale/latency after the final integration: latency gate passed 4 tests in 24.80 seconds after correcting a Records-attributable warm-recall regression. Candidate lanes now reuse the exact checkpoint-bound recall projection while live policy checks remain immediately before page hydration, frame-parent metadata access, and final egress. The 8,000-note graph test fell from 191,760 policy walks to bounded live hydration/egress checks and passed in 14.50 seconds; a deterministic 200-note guard measured exactly 210 checks. A second CI regression showed that graph consistency checks were materializing the same O(corpus) request-path projection after each semantic write even though they needed only its identity; those checks now consume the exact event-maintained recall checkpoint directly. Cold checkpoints retry if the access policy changes or a live registry appears during projection, preserving the prior fail-closed identity guarantee. The final semantic write gate at 2,000 pages measured validation median/p95 12.5/13.6 ms and commit median/p95 43.2/47.2 ms; at 8,000 pages, 49.0/50.2 ms and 110.2/149.4 ms. Cold graph activation remained outside measured samples.
- Independent review found and drove corrections for hidden identity/title collisions, authorization-before-resolution, bounded link candidate work, disclosure receipts, undeclared manual fields, Planning-profile refusal, tracker claim scope, exact graph and resolver checkpoint advancement, direct delete publication ordering, complete watcher publication retry, persisted graph read barriers, cold/foreign-lineage source and topology proof, proof stabilization, descriptor-rooted reads, and Windows ancestor namespace pinning. Native probing showed that `FILE_SHARE_READ` with zero access did not reserve the namespace; the final least-privilege `FILE_LIST_DIRECTORY + FILE_SHARE_READ` combination blocks rename/delete/reparse substitution. Two independent post-CI reviews also approved the checkpoint-projection optimization, including policy-only invalidation, cache races, parent-sidecar TOCTOU handling, live hydration, and live egress, with no open findings.
- Independent generated-surface verification passed: copied real X3 fixtures retained all four canonical hashes; ordinary Push/Pull template insertion, safe append, targeted update, structured and derived views, direct-edit audit gaps, opaque Planning links, and recall isolation worked through CLI and MCP. The unrelated vehicle fixture enforced mixed-release governance before aggregation and targeted only the corrected item; the CSV fixture invalidated stale continuations and refused mutation. Its focused public-surface suite passed 73 tests.
- Durable capture appended the delivered boundary, verification, deferred work, and publication failure modes to `Knowledge Base/Notes/Research/Exomem/exomem-needs-first-class-mutable-records-for-continuous-tracking.md`.
- Delivery: committed on `feat/first-class-records`, integrated `origin/main` at `9665d31`, pushed the branch, and opened ready PR [#405](https://github.com/Artexis10/exomem/pull/405) without merging it.
