...
- [x] 6.2 Run one phase-boundary full test suite, strict OpenSpec validation, the public-artifact privacy gate, and explicit mutation-tripwire cases for the new parser/resolver branch and precedence predicates; record exact evidence.
  - Evidence: GitHub Actions run `33399406122` attempt 2 passed all eight core shards, all four harness shards, Windows NTFS, TUI, E2E, onboarding, packaging, lint/types, capability, and OpenSpec jobs. OpenSpec 1.10 reported 169 passed and 0 failed; the public-artifact privacy gate reported 3,475 files and 3,543 text payloads clean. Focused parser/resolver mutation tripwires passed, including repeated-key attribution, conflicting-duplicate refusal, marker topology, precedence, and unrelated valid-contract resolution. The first run's isolated `/proc` race passed on the failed-job rerun and in the final full run.
- [x] 6.3 Have a fresh author-independent reviewer reproduce the user journeys and adversarial cases against the integrated diff; return all findings to the implementation lane and rerun the same reviewer after corrections.
  - Evidence: the fresh reviewer returned duplicate-key, marker-topology, bootstrap-vocabulary, and packaged-plugin findings to the implementation lane; after corrections, the same independent lane reproduced the adversarial cases, ran 86 focused tests, compared generated plugin contents byte-for-byte, and returned no findings with a Ready verdict at `546b6965`.
- [x] 6.4 Synchronize the delta specs, archive the completed change through OpenSpec, and rerun `openspec validate --all --strict` before delivery.
  - Evidence: OpenSpec 1.10 archived the change as `2026-08-31-add-user-authored-workflow-contracts`, synchronized six capability specs, preserved the existing software/OpenSpec compatibility scenarios, and the post-archive strict validation reported 169 passed and 0 failed. The archive-discipline check reported 57 active changes with no task-complete archive debt.
