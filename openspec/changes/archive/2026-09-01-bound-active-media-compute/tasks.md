...
- [x] 6.1 Run `openspec validate --all --strict`, focused changed-behavior tests, lock/install checks, and Ruff locally; use the pull request's required CI for the full lean corpus and verify it is green or document only demonstrably unrelated baseline red with failure-name evidence.
...
- [x] 6.4 Commit only the intended scope, integrate current `origin/main` safely in the isolated task workspace, push the branch, and open a ready Conventional Commit pull request containing rationale and verification evidence; keep the workstation containment drop-ins until the released build is deployed and measured.
