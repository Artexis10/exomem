...
- [x] 1.1 Add a parity test (`audit-vs-usage.py` extraction) proving `usage._relevance_canon`,
...
- [x] 1.2 Add formula unit tests for `usage_multiplier`: bounds (`mult` strictly in
...
- [x] 1.3 Add a keyword/hybrid-path integration test using JSONL fixtures written directly to a
...
- [x] 1.4 Add a never-creates-candidates test: a page with maximal usage activation but no
      lexical/semantic/graph match to the query stays absent from `prefer_used=True` results.
- [x] 1.5 Add dominance-invariant tests: a superseded page at maximum usage activation
...
- [x] 1.6 Add cold-start no-op tests: no `logs/` directory, or all three JSONL files empty/absent
      → every `usage_multiplier` call returns exactly `1.0`.
- [x] 1.7 Add disabled-signal no-op tests: `KB_MCP_DISABLE_USAGE_BOOST`,
...
- [x] 1.8 Add snapshot invalidation tests: a `UsageSnapshot` reflects new events after a log file
...
- [x] 1.9 Add a rerank-path test: with `rerank=True` and `prefer_used=True`, a hit's
...
- [x] 1.10 Add a `ranking_config.json` roundtrip test for the six new knobs
...
- [x] 1.11 Add a query-log test: `log_find_call` records `prefer_used` in
      `logs/queries.jsonl` the same way it already records `prefer_compiled`.
- [x] 1.12 Add a hot-cache bypass test: two identical `find` calls with `prefer_used=True` never
...
- [x] 2.1 Create `src/kb_mcp/usage.py`; move `_relevance_canon`, `_relevance_read_jsonl`, the
...
- [x] 2.2 Update `audit.py` to import and delegate to `usage.py` for these functions; keep
      `_check_stale_review` and the rest of the stale-review check otherwise unchanged.
- [x] 2.3 Add `UsageSnapshot` (dict[canon_path, B], memoized on log file sizes+mtimes and current
      date, refresh TTL via `KB_MCP_USAGE_REFRESH_S`, default ~300s).
- [x] 2.4 Add `usage_multiplier(canon_path, snapshot, config) -> float` and
      `reset_usage_cache()`.
- [x] 2.5 Add the `KB_MCP_DISABLE_USAGE_BOOST` kill-switch, checked before any log parsing.
- [x] 2.6 Run `tests/test_audit_stale_review.py` and the new parity test (1.1) to confirm
      byte-identical stale-review behavior post-extraction.

## 3. Usage Boost Formula and RankingConfig Knobs

- [x] 3.1 Add `usage_boost`, `usage_decay`, `usage_horizon_days`, `usage_w_surfaced`,
...
- [x] 3.2 Implement the `B = ln(Σ w_j·Δt_j^(-usage_decay))` / logistic-boost formula in
...
- [x] 3.3 Confirm `ranking_config_from_jsonable` requires no changes for the new float fields
      (verify via test 1.10); do not add bespoke coercion code.

## 4. Find Plumbing

- [x] 4.1 Add `prefer_used: bool = False` to `find()` and `_find_semantic()` in `find.py`,
      symmetric with `prefer_compiled`/`prefer_active`, with matching docstring language.
- [x] 4.2 Apply the usage multiplier as a 4th post-RRF multiplicative boost alongside
...
- [x] 4.3 Re-apply the usage multiplier to `Hit.rerank_score` after the CrossEncoder pass
      (`find.py:1029-1044`), appended after the existing `prefer_compiled`/`prefer_active`
      re-apply blocks.
- [x] 4.4 Extend `Hit.as_dict()` to add `activation` (raw `B`, rounded) and `usage_boost`
      (multiplier applied) to `signals` only when `prefer_used` is active and the multiplier is
      not `1.0`.
- [x] 4.5 Bypass the hot `find` cache entirely when `prefer_used=True` (no cache read, no cache
      write); leave `prefer_used=False` cache behavior untouched.

## 5. Command Surface, Logging, and Docs

- [x] 5.1 Add `prefer_used: bool = False` to `op_find` (`commands.py:246-395`), threaded into
...
- [x] 5.2 Extend `query_log.log_find_call` to accept and record `prefer_used`, matching the
      existing `prefer_compiled` field.
- [x] 5.3 Regenerate `tests/fixtures/mcp_tool_schemas.json` for the `find` tool's new
...
- [x] 5.4 Amend the two `audit.py` docstrings that assert activation never touches `find` ranking
...
- [x] 6.1 Run `KB_MCP_DISABLE_EMBEDDINGS=1 uv run python -m pytest -q` and confirm the full
      suite is green, including the disabled-signal no-op tests (1.7) that exercise this same
      env var.
- [x] 6.2 Run `uv run ruff check`.
- [x] 6.3 Confirm a default (`prefer_used=False`) `find` call's output is byte-identical to a
      pre-change baseline fixture.
- [x] 6.4 Confirm no new abstraction beyond `usage.py`/`UsageSnapshot` was introduced, and no
