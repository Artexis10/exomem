...
Lifecycle graph-epoch abort uses the same native directory-flush primitive after removing a floor or checkpoint that did not exist before the attempted transition. Only a successfully flushed exact restoration may return the original lifecycle refusal. An unlink or flush failure remains `GraphLifecycleRollbackError` and reconciliation-required; it is never swallowed or mislabeled as a successful rollback. POSIX retains its current descriptor/fsync barrier.

`atomic_rename` also distinguishes a refusal before rename from a durability failure after the canonical rename already occurred. The graph lifecycle transition records that post-rename state before propagating the failure. Abort must then durably inverse-rename before restoring the prior graph epoch. If inverse rename or either directory barrier fails, it retains the staged graph floor/checkpoint and lifecycle evidence and reports rollback/reconciliation required; it must not erase freshness evidence for a canonical placement that may have changed.
