...
The rule, now stated in the spec, is deliberately narrow: **a queue that fires on a date the author wrote down outranks every queue that infers its own candidates.** `bridge_review` fires on a governance review date someone committed to; `prediction_window` fires on an epistemic check date someone committed to. Placing an authored deadline behind an inferred candidate would rank a guess above a promise.

It sits after `bridge_review` rather than before it because a bridge review carries a commitment to another audience, where a check date is a commitment to oneself; when both tie, the one with someone else waiting on it goes first.

An earlier draft of this decision claimed the whole list runs on a gradient from authored to inferred. That claim is false and has been withdrawn: `unprocessed_source` and `relation_debt` are deterministic scans over authored state, strictly less inferential than the `corpus_contradictions` cosine band sitting above them. The existing relative order of those four is historical, it is not being changed here, and the spec now says so rather than inventing a rationale that would license inserting a future queue anywhere in the list.
...
**Promoting this queue to the default made a latent composition defect routine.** `attention._rank` anchors a partitioned finding on `path\0partition`, which shares no key with the same page's unpartitioned findings — so a note flagged by both took two rows and its RRF votes never summed. That was harmless while `bridge_review` was the only partitioned queue, because it anchors on governance grant paths that no page-level queue flags. Predictions live on exactly the `insight` and `research-note` pages `stale_review`, `relation_debt`, and `corpus_contradictions` already flag, so the collision became ordinary. Fixed in the composition rather than papered over in the spec: a path's unpartitioned findings now fold into each of its partitioned items, contributing both reasons and votes, and never form a separate row. The `Multi-Signal Additivity With Dedup By Anchor` requirement is modified to state that fold rule normatively, because it was previously reconciled with partitioning only by a parenthetical in a different requirement.

**The capability's `## Purpose` paragraph enumerates five queues by name and will be stale once this change is archive-synced.** A MODIFIED delta reaches requirements, not the capability preamble, so nothing in this change can update it. Whoever runs the archive sync should rewrite that paragraph to name six, and the `bridge_review`/`prediction_window` split between authored-date queues and inferred ones is the sentence to carry over. Flagged here because a stale `## Purpose` is exactly the class of drift this branch exists to fix elsewhere.
