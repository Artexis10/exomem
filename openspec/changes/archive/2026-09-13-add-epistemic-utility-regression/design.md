...
The architecture serves both coding and conversational work. Coding outcomes are observed artifacts and executable checks; conversational outcomes include continuity, current constraints, grounded decisions and committed actions. Report those workload families separately. The first shared configuration/action world validates the instrument; it does not establish real coding performance or open-ended conversational quality. Follow-up canaries must include both workloads, with memory attentiveness and context policy measured independently.

The initial fixture exposes authoritative facts in one common tool call. It therefore gives memory little opportunity to reduce research work: neutral outcomes cannot establish neutrality on knowledge-intensive tasks. The adopted native API actor removes redundant text only when parsed JSON equals the structured payload and the text carries no distinct metadata; raw and delivered results remain in the trace. Record that renderer in the manifest and interpret its context measurements as native-actor costs; Claude Code and Codex workflow claims require their own canaries.
...
The next implementation should complete phases 1 and 2 together as one narrow feature. Those phases are the acceptance boundary for this change; phases 3 and 4 are a sequenced roadmap requiring follow-up OpenSpec changes, not open acceptance tasks here. Shipping only a synthetic scoreboard would leave the product question unanswered. Implementation extends this reviewed plan. The repository permits squash merges only; an acknowledged receipt must name a strict ancestor containing the amended document. Therefore the ready implementation PR carries a pending f32 receipt, followed by acknowledgment against the merged contract revision and the bounded live smoke. Instrument and isolated semantic-runtime preflight results cannot substitute for that comparison. No live utility result is claimed before the release gate opens.

## First live smoke: identifier contract repair

The v1 seed-11 smoke at `3677c2e7` completed eighteen sessions for $0.01278401, with no capture, maintenance or retrieval. Both self-contained actors copied an explanatory clause into the constraint string, and the exact-state grader rejected it. Generator v2 must separate canonical identifiers from natural-language explanations; tool field documentation must make exact copying explicit. The grader remains strict and the original v1 artifacts remain immutable diagnostic evidence. Details and original artifact digests are in `docs/benchmarks/utility-smoke-2026-09.md`. Zero memory use stays in outcome accounting and is never an infrastructure-invalidity gate. No live v2 result or general product utility claim is established by this repair.
