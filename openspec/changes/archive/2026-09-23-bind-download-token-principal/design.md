...
- **Graph seeds are decided by their parent.** `egress.unit_parent_withheld` collects every page the resolver can consult for the reference: all of the graph's parent-ref rows for it, current or not (each row reports drift about its own page), and the node rows carrying the reference (`epistemic_graph.unit_ref_indexed_paths`). It adds the pages the reference's memory id names in the reference index, or the page an `exomem://vault/` or `exomem://source/` parent names by path, which is how an id-less page's units are referenced — that path-named candidate is confined to the vault the same way any caller-named path is (`vault.resolve_under_vault`) before it is ever decided, and a name that does not resolve inside the vault counts as withheld without being stat'd or read. Each candidate is decided at `RELEASE_FLOOR`, the level below which the graph guard already withholds a seed. An explicit sub-floor decision counts as withheld for every caller, the owner included, so a rule naming the `owner` audience still withholds the owner's seed. An UNDECIDABLE candidate — a walk with more rows than the resolver examines, or a stale row naming a path that is no longer there — also counts as withheld, but not for the owner: that signal is a graph artifact rather than a policy decision, and must not cost the owner the stale-status and drift report the unguarded answer carries. A withheld page makes the seed resolve as `UNRESOLVABLE_UNIT_REF`, so resolution, parent validation and drift accounting take the absent-page branch, while the response still echoes the caller's own reference. A page seed on `connect_memory` context takes `op_get`'s page decision.
...
- **Path-named seed confinement.** The `exomem://vault/` and `exomem://source/` candidate above is decoded before it is confined, so an absolute path, a `../` traversal or a device reference had to be rejected before it could reach a decision — otherwise the decision itself would stat and, for a markdown-shaped result, read whatever it named, anywhere the server process can reach. It is now resolved under the vault root exactly as a caller-supplied path is everywhere else in the release plane. As a second boundary, the receipt writer that hashes a released item's bytes for evidence also confines its candidate before reading it, so an unconfined candidate that reached that far would lose its content hash rather than have its bytes read.
...
- For a caller other than the owner, a stale graph index (rows for pages that no longer exist, or more rows than the resolver examines) makes a seed answer as absent, where the unguarded answer would report the staleness. That is the conservative side of the decision, and for the owner only that undecidable-candidate signal is exempted: a rule that names the `owner` audience still decides the owner's seed the same way it decides any other caller's.
- A memory id that names both a released and a withheld page (a duplicate `exomem_id`) makes a non-owner's memory-id-named unit seed answer `missing` rather than `found`: the withheld candidate decides the walk before the released one is reached. That is a weaker existence signal than the ordinary reference resolver gives for the same duplicate (`ambiguous`, not `missing`), but it stays on the conservative side of withheld == absent and discloses nothing about either page's content. Chosen over resolving among released candidates only, which would need the candidate set itself to carry each candidate's release level rather than a bare path, for a case that is already a pre-existing content-integrity defect (a duplicated identity) independent of this change.
