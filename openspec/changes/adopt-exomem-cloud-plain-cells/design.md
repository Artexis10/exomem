...
- **Complete platform Secret bundles.** A `sops_k8s_secret` destination may declare `value_shape: json-object` and `key_sets` in place of its legacy scalar `key`. The named destination's declared sets are the transport's exact allowlist. Cloud declares cell-token `{current, currentVersion}` or `{current, currentVersion, previous, previousVersion}`, backup-master `{keys, currentVersion}`, and B2 key-management `{keyId, applicationKey}`; the generic transport does not embed these field names. The source is one bounded, flat JSON object of nonempty, single-line string values from stdin or BWS. Its keys must equal one declared set; partial rotation pairs, duplicate keys, nested values, placeholders and extra fields fail closed. One complete object becomes one immutable versioned SOPS Secret artifact and one registry entry. Every `stringData` leaf must be encrypted, and a verification decrypt must equal the full original document before publication. Apply validates the decrypted shape and converts `stringData` to base64 `data` in memory for server-side apply; the stored artifact remains SOPS ciphertext. After apply, a live Kubernetes projection emits only the Secret's data key names; those keys must equal the selected set before activation succeeds, including when another field manager retains an old rotation field. No forced conflict takeover is allowed.
