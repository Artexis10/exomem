...
Capability filtering remains honest: every active profile that advertises Records must export `record_memory`, including the disposable hosted-cell acceptance surface. Lifecycle acceptance uses the additive `hosted-alpha-agent-v2` profile, which exposes the canonical nine-action selector and is bound to Records reader version 2. The immutable `hosted-alpha-agent-v1` profile remains valid for existing clients with unchanged membership, identity, locks, and registered evidence; it does not silently acquire lifecycle selectors. A profile that intentionally excludes the command reports Records unavailable instead of teaching a dead route.
...
Supplying both selectors or neither is refused. Revision validation and targeted inspect return the closed lifecycle guard object `{"expected_manifest_hash":"<sha256>","expected_container_hash":"<sha256>"}` and a normalized proposed contract but write nothing. `revise` accepts exactly `expected_manifest_hash`, `expected_container_hash`, and `why`; `rebaseline` additionally accepts exactly `acknowledged_gap_codes`.
...
`revise` accepts a collection selector, complete proposed manifest text, `expected_manifest_hash`, `expected_container_hash`, and `why`. The proposed text may omit system-owned `record_audit`; if it includes one, it must exactly match the current marker. Under the existing writer boundary the system authorizes and re-reads the current manifest, validates the proposed contract and all current canonical items, rechecks the guards, derives the next audit version/head itself, and publishes the manifest plus that Records audit transition atomically.
...
`rebaseline` handles a collection that is currently structurally valid but whose audit hashes differ because a human or generic editor changed canonical files. It accepts `expected_manifest_hash`, `expected_container_hash`, the exact inspect-reported `acknowledged_gap_codes`, and `why`. It writes no item content. It appends a content-free checkpoint transition and updates the system-owned manifest audit head atomically.
...
Every Records-affecting pull request runs a deterministic installed-wheel test in a temporary vault. Unlike the current fixture-first loop, the client must call `describe`, validate and create a manifest containing a saved view without filters, inspect it, append/query/update, revise it, create a direct-edit gap, rebaseline it, restart, and prove parity. This is the local code gate only; it cannot claim deployed HTTP/OAuth/client acceptance. A concurrent full graph rebuild test proves an unrelated Records mutation is not blocked once the coordinated graph change lands.
...
The live runner emits content-free unsigned facts. The existing hosted promotion verifier accepts them only inside its operator-signed, 24-hour evidence envelope. The evidence is an exact closed object whose executable nested schema contains only deployment/release/package/surface bindings, `hosted-alpha-agent-v2`, and reader floor 2; run, reset, principal, and audience identities; exact client/model/system-contract and prompt-case bindings; required action and restart results; per-mutation request/receipt/committed-terminal correlations; independent before/after readback hashes; and graph-availability proof digest. Extra nested fields, stale evidence, reset reuse, identity mismatch, wrong profile/reader floor, unverifiable readback, or an incomplete case/action set fail closed. Exact replay of byte-identical signed evidence against its unchanged candidate is an idempotent no-op returning the same promotion result; it never creates a second acceptance. `src/exomem/hosted_plugins.py`, `plugins/hosted/promotion/*.json`, `deploy/chatgpt/personal-plugin-contract.json`, and their guardrail tests are the enforcing artifacts. CI does not pretend it can prove a not-yet-deployed endpoint: local installed-wheel proof gates the code PR; signed post-deploy live HTTP/OAuth and client evidence gates the later promotion PR. Current hosted v1 registered evidence remains unchanged; the changed Records surface stays pending until v2 proof is present.
...
3. Generate and deploy the additive `hosted-alpha-agent-v2` candidate with its reader-v2 deployment lock; do not alter the existing v1 candidate, profile, or evidence.
4. Deploy the combined release to the public service without changing existing collection bytes.
5. Reset or provision the dedicated disposable acceptance vault and run deterministic HTTP/OAuth plus Codex/Claude acceptance through v2.
6. Open a new promotion PR carrying structured current v2 evidence and supersede PR #438.
7. Existing collections require no migration. Collections with valid direct-edit audit gaps may opt into `rebaseline`; invalid optional manifest detail uses guarded `revise`.

The release establishes Records reader contract version 2 before enabling either lifecycle selector. The first v2 transition atomically upgrades the manifest's existing `record_audit.version` from 1 to 2, which is the durable per-collection reader marker. Every later append/update preserves that marker even though its already-closed event and receipt remain version 1, so a valid history may be `v1 -> v2 -> v1`; later lifecycle events remain v2. The new reader dispatches each event by its own version, scans the whole reachable chain, and never loses an earlier acknowledged discontinuity. The immediately preceding reader fails closed on the v2 manifest and never rewrites it. An additive new deployment-lock version/schema records `minimum_records_reader_version: 2` for the new `hosted-alpha-agent-v2` lifecycle candidate without mutating the closed existing lock v2 or any v1 Hosted lock. Readiness/rollback tooling refuses a v2 runtime below that floor. Supported rollback is therefore a compatibility build retaining reader-v2 parser/status semantics while disabling `revise` and `rebaseline`; it is never the predecessor binary. Existing v1 clients remain valid against their unchanged profile and do not advertise the lifecycle selectors. Upgrade, first-write, append/update, restart, downgrade-refusal, and rollback-build fixtures pin that behavior.
