...
spread gate of two distinct eligible pages while resolving against neither an
existing vault page nor the entity registry's titles and aliases. Pages named
`index.md` or `log.md` list things rather than reach for them: they SHALL NOT
supply spread and SHALL never anchor a finding. The finding SHALL carry reason
...
SHALL add no embedding, no model call, and SHALL never create any page.

#### Scenario: A recurring unresolved identity becomes a candidate

- **GIVEN** two distinct pages whose bodies link an identity that exists neither as a page nor as a registry title or alias
- **WHEN** the audit sweeps the category
- **THEN** one `entity_recurrence` finding is produced carrying reason `unresolved_identity_recurs`, the candidate, the two pages sorted, and the near-match list

#### Scenario: Frequency inside one page is not spread

- **GIVEN** one page linking the same unresolved identity five times and no other page linking it
- **WHEN** the audit sweeps the category
- **THEN** zero findings are produced for that identity

#### Scenario: Navigation pages are not evidence

- **GIVEN** an unresolved identity linked from three `index.md` pages and from one ordinary page
- **WHEN** the audit sweeps the category
- **THEN** zero findings are produced for that identity
...
- **GIVEN** an identity linked from one eligible page and from one page that is superseded, archived, draft, or in an excluded access tier
- **WHEN** the audit sweeps the category
- **THEN** zero findings are produced for that identity
