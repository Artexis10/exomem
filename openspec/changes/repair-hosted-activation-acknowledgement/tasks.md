...
- [ ] 1.4 Obtain independent adversarial/security review of the authority, secret-disclosure, recovery and rollout design. Resolve findings before changing those boundaries.
  ...
  - [ ] 1.4.2 Deferred from the same review, each needing a siting decision rather than a patch: no deployment preflight calls `validate_activation_ack_server_certificate`, so the test-CA marker and the remaining-lifetime floor are inert and nothing notices a certificate expiring (review finding 3); the classifier has no production caller and its legacy-uncertain verdict is stateless, so the "never reconcilable into looking clean" invariant needs a durable mark rather than a docstring (finding 4); and the floor-2 refusal is sited at runtime while it reads as configuration-time, so the tenant discovers it as a 409 rather than the operator discovering it at bind time (finding 5).
