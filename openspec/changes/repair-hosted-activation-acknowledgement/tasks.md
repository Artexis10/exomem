...
- [ ] 2.1 Extend writer_lease.py with attempt-bound PreparedCanonicalMutationRecovery and durable private prepared payloads. Test tampering, substituted attempts, stale payloads, missing evidence and precommit crash without marking preparation as success. Surveyed 2026-09-20: the type, its v1 contract and 16 tests live in `hosted_mutation_journal.py` and `tests/test_hosted_mutation_journal.py`, covering tampering, substituted attempts, forged candidates and incomplete child sets. What is missing is the `writer_lease.py` side — nothing there references the type — plus stale-payload, missing-evidence and precommit-crash cases.
...
- [ ] 2.5 Prove the crash cuts before commit, after publication/before acknowledgement, after authoritative acknowledgement/before response, and before outer terminal persistence.
  ...
  - [ ] 2.5.2 The remaining cuts: after authoritative acknowledgement before response, before outer terminal persistence, composite mutations with an incomplete child set, and the three Adoption Studio boundaries. Include composite mutations with an incomplete child set and fresh-process recovery, not only caught exceptions. For Adoption Studio cover after catalog/before proposal completion, after atomic proposal/receipt replacement before SQLite evidence, and after aggregate commitment before terminal persistence; none may repeat _route_apply, duplicate a note, or advance activation to record completion.
