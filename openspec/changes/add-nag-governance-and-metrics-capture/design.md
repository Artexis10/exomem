...
`due_total` has exactly one writer, the delivery path, and therefore exactly one meaning: the size of the block a caller was handed. It exists because `emissions=0, writes=12` is equally the shape of governance working and the shape of a batch that delivered nothing, and the counter assertion must be able to tell those apart. A second writer would defeat it, and one was tried: recording reconcile's own unfiltered recompute under the same name let a pre-dismissal count stand as the denominator for a later batch that owed nothing at all.
