...
The `close-memory-loop` integration extends opt-in scope with explicit vault-wide `edge.add`: existing and subsequently committed canonical endpoints can be linked when both belong to the authenticated logical vault. Existing project-edge grants retain their fixed resolved scope. This is a versioned scope extension, not permission inferred from `entity.create`, a note, a project label or an upgrade. A trusted owner must deliberately issue the broader grant; existing grants do not widen. The grant format/reader floor must distinguish the new scope so older runtimes cannot reinterpret it. Combined create-and-link payloads must prove the new identity's vault ownership inside the same leaf boundary or use separately receipted create then link; an unresolved prospective label is never an endpoint proof. Role/convention edits and other future actions remain outside these additive actions.
