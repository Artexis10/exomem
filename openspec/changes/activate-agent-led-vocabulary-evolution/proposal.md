...
- Support an explicitly issued vault-wide edge grant for existing and future canonical endpoints in that vault, as required by `close-memory-loop`; retain narrow existing grants and enforce a compatible runtime floor without automatic activation or scope widening.
