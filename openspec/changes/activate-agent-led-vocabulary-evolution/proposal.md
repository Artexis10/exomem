...
- Make ordinary conversation the entry point for intent, preference changes and on-demand permission control. Dashboards provide visibility, history and transparent inspection; routine work must not require dashboard navigation. Supported client confirmation and browser fallback adapters share the same exact review and authority contracts.
- Integrate with the existing prominence and delegation envelope so engagement, interruption and authority remain distinct controls. Portable user profiles and cross-client refresh require extensions owned by those capabilities; this change consumes them rather than introduce a vocabulary-only settings system.
