...
- [x] 1.1 Add a regression test that registers a slow corpus-context flight and asserts a
...
- [x] 1.2 Add a test asserting an owner build error still raises for the waiter, so the
  bound cannot be implemented by swallowing failures.
- [x] 1.3 Record the measured baseline (98% post-commit share, 103.6s median) in the test
  module docstring so a future reader can tell whether the numbers still hold.
- [x] 1.4 Add a regression test proving an already-settled flight returns immediately
...
- [x] 2.1 Introduce the exact 2.0-second interactive bound as a fixed module-level
...
- [x] 2.2 Add the already-settled fast path, then replace `flight.done.wait()` at
  ...
  state afterwards.
- [x] 2.3 On expiry, return the existing retryable `MUTATION_WARMING` outcome with
...
- [x] 2.4 Leave the owner's build, registry cleanup, and `flight.done.set()` paths
  untouched; assert by test that an expired waiter does not disturb the owner.
- [x] 2.5 Preserve the existing `same_inputs` recomputation branch exactly.

## 3. Make the deferred outcome visible

- [x] 3.1 Reuse the existing pre-commit `MUTATION_WARMING` carrier rather than adding an
  optional corpus value or a committed terminal-sync field.
- [x] 3.2 Surface code `MUTATION_WARMING`, status `retryable`, `committed: false`, and
  `retry_after_ms: 2000` in the default response projection, not only under
  `response_detail="full"` or `"legacy"`.
- [x] 3.3 Add a test asserting timeout returns that exact default projection and that no
  canonical mutation occurs.
- [x] 3.4 Audit consumers of `build_corpus_context_with_census` for paths that assume a
...
- [x] 4.1 Add an enumeration test that walks request-reachable blocking joins and requires
  each to be bounded or carry an explicit background-only declaration with a reason.
- [x] 4.2 Declare `file_watcher.py:426` and `media_worker.py:275` background-only with
  recorded reasons, or bound them if the audit shows they are request-reachable.
- [x] 4.3 Assert the test fails when a new unbounded request-path join is introduced.

## 5. Verify against the real workload

- [x] 5.1 Run the existing write-latency gates and confirm no regression in commit-path
...
- [x] 5.5 Run `openspec validate bound-corpus-context-flight-join --strict` and
