...
**Hosted plugin note.** `observe_memory` is a member of the `hosted-alpha-agent-v1` hosted profile, whose committed compatibility descriptor is *derived* from the live command registry, so moving the surface moves that descriptor's identity. An earlier revision of this proposal left every hosted packet frozen and recorded the resulting red tests as an intended escalation signal. That escalation was answered: the decision is that the derived identity tracks the surface, because a descriptor pinned against a schema it no longer serves would advertise a contract nobody serves. This revision therefore re-renders, and the red tests it previously described are green.

Four derived surfaces move, all of them for the same reason and all verified to be confined to `observe_memory` gaining `verdict`, `check_by`, and `id`: the v1 compatibility descriptor and its two platform locks, the same three for the `hosted-alpha-agent-v2` candidate, and the `claude-connector` and `claude-plugin` directory packets. Command membership, surface fingerprints, skill digests, definition digests, promotion records, and every package archive are unchanged. v1 promotion state is `pending` and no directory channel has an active submission, so no live registration or published listing is affected. The `openai-plugin` channel is deliberately untouched: it was already stale on `origin/main`, and regenerating it here would sweep unrelated pre-existing drift into this change. See `design.md` for the full reasoning and the separately-filed alternative.
