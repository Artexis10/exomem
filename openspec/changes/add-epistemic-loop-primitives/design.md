...
- **The hosted plugin's frozen `hosted-alpha-agent-v1` descriptor and the live tool surface now disagree, and this change cannot resolve that by itself.** `observe_memory` is a member of the v1 hosted profile, and `hosted_plugins.compatibility_manifest` derives the committed descriptor from the *live* command registry filtered by that profile. So moving `observe_memory`'s schema necessarily moves `compatibility_sha256` — which `hosted-plugin-identity` explicitly requires ("a schema digest change → `compatibility_sha256` changes") and `check_compatibility_descriptor` enforces. But `tests/fixtures/hosted/v1-release-identities.json` pins those same derived bytes as immutable, and `hosted-client-plugins` calls v1 "the existing immutable profile". Two committed requirements, one artifact, opposite demands. This change leaves every hosted packet byte-frozen and reports the collision rather than picking a winner: rewriting a release-identity fixture to go green would be exactly the silent falsification this proposal exists to prevent elsewhere. Whoever owns the hosted release plane decides whether v1's derived identity may track the surface (regenerate the descriptor and both platform locks, update the two derived fixture entries; the shipped `claude.zip` bytes are unaffected and v1 promotion is still `pending`, so no live registration breaks) or whether the frozen candidate should stop deriving its descriptor from the live registry.
