...
- **Preserve-on-omit for the new arguments but replace-on-omit for `tags` / `context` / `relations` is an inconsistent contract.** The asymmetry is a wart this change inherits rather than creates, but it would have become a trap: a first draft of the tool help claimed an update "never discards an authored metadata row it was not given", which is false for exactly those three, and that help is digested into the shipped tool surface — so the contract would have asserted the opposite of the behaviour. The help now scopes the preserve claim to the governed keys plus unowned rows and states replace-on-omit explicitly on all three, with a test pinning both halves against the generated schema. Unifying the semantics still belongs to a change scoped to alter shipped argument behaviour; until then the help must not paper over the seam.
