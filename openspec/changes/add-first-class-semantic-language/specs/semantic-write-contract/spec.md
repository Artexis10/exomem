...
### Requirement: Reviewed-None Creation Is Atomic And Content-Bound
Creation-capable governed writers SHALL support `validate_only=true`. Validation SHALL preassign a candidate page UUID and return `draft_id`, `draft_hash`, deterministic relation findings/candidates, and no filesystem mutation. A reviewed-none commit SHALL repeat identical proposed content and supply the unused `draft_id`, `relation_disposition="reviewed_none"`, matching `relation_review_hash`, and a non-empty `relation_review_reason`. The writer SHALL recalculate the hash over candidate identity plus normalized final content, reject mismatch/reuse, and atomically commit page identity/content with portable fingerprint-bound review state. Portable state SHALL live in governed Markdown/frontmatter or a vault-managed governed review artifact in the same file batch, not solely in a derived database, and SHALL survive index rebuild or vault transfer. A qualifying typed relation or genuinely empty-corpus bootstrap SHALL not require a reviewed-none token.
...
- **THEN** the page and its portable review state commit atomically with a current disposition
