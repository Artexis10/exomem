...
A candidate span is the maximal adjacent span on the same Markdown clause side of the cue or predicate. It contains 1–8 Unicode letter, number, or mark tokens joined only by single whitespace, hyphen, apostrophe, or ampersand; is 2–128 UTF-8 bytes after NFKC normalization; and ends at `.`, `,`, `;`, `:`, `!`, `?`, a Markdown hard break, or the frame delimiter. Pronoun-only, all-numeric, stopword-only, URL, email, filesystem-path, inline-code, code-block, Markdown-link-target, and exact registry-cue-only spans are rejected. The only additional date/time exclusions are the exact NFKC-case-folded whole-span shapes `\d{4}-\d{1,2}-\d{1,2}`, `\d{1,2}:\d{2}(?::\d{2})?`, `(?:[01]?\d|2[0-3])h[0-5]\d`, and `\d{1,2}(?:(?: |:)\d{2})? ?(?:am|pm)`; no natural-language calendar vocabulary participates. Capitalization and script are never span conditions. A clause with an unsupported trailing adjunct is not shortened speculatively; it simply may not recur under the same normalized identity.

The grammar does not pretend to decide whether an otherwise valid word or phrase is semantically a date, generic description, or identity. Those interpretations are indistinguishable when the authored bytes and structural frame are identical. Instead, `typed-copula`, `typed-label`, and `body-field` are the only **identity-witness frames** in v1. `subject-relation` and `identity-relation` are **facet-only frames**: they may enrich a directly witnessed identity, and they may produce hydration or ambiguity pressure when the normalized identity already resolves to one or more active Entities, but relation rows alone never seed promotion pressure for an unresolved identity. Thus unresolved relation-only repetitions such as `I work with today` or `I work with one organization` remain quiet without an English phrase blacklist, while an explicit ordinary declaration such as `Juniper Circle is a community` can witness a missing identity and later relation frames can supply reusable facets. For an existing `Juniper Circle` Entity, exact resolution itself is the identity warrant, so later relation-only facts can produce hydration rather than leaving it stale. A deliberately unusual explicit declaration such as `organization: Today` remains bounded evidence for the active agent to interpret; the sensor does not claim semantic truth.
...
An origin is the contributing Source reference where provenance exists, otherwise the stable session or page reference. Derivatives of one Source count once. Ordinary-text evidence requires at least three eligible pages in three independent origins and at least two distinct material facet atoms from at least two origins. Promotion additionally requires at least one identity-witness row from `typed-copula`, `typed-label`, or `body-field`. One or more active exact/alias Entity matches instead supply the identity warrant for hydration or ambiguity, allowing qualifying relation-only facets to surface stale existing Entities without reopening unresolved relation-only false positives. A direct witness may be lowercase or non-Latin and may use any active vault-defined type cue; either warrant is a structural reason to ask the active agent, not proof of semantic truth or a selected target.
...
Candidates report identity and display, eligible page/origin/facet totals, bounded context references and excerpts, facet hashes, observed role or membership language, resolved co-occurring Entity refs, active and unresolved type cues, exact/alias/near-match evidence, grammar and predicate-table identity, and evidence and registry fingerprints. Every projected sample is deterministically bounded while full, returned, and omitted counts remain visible. This includes top-level and nested context `type_cues`, `active_type_cues`, and `entity_families`; full internal cue/family sets continue to drive compatibility and fingerprints before their response projection is capped.
...
Hydration continuation is a separate identity-bound budget: after each separately confirmed batch returns a terminal mutation receipt, exactly one recheck may request only that same identity. Rechecks after processed batches 1–7 may return at most the next eight contexts; the recheck after processed batch 8 is closure-only and may return only closed or `deferred_remaining_count`. A newly returned batch authorizes no further call until it receives a fresh exact confirmation and terminal receipt. The session mutates at most eight batches and performs at most eight identity rechecks, leaving any remainder open for the next session's ordinary read. Off and light remain explicit-request-only. The hookless custom-instructions projection is bounded by the 1,500-byte web custom-instructions limit, so it carries the cadence and the unavailable-skip rule and defers the decision order to bootstrap, which carries it; the scaffold and core skill teach the full decision order. If the active surface cannot request an explicit category or attention read, the carrier says the check is unavailable and skips it; it never substitutes a local scan or a model guess.
...
- [Lexical false positives] -> Require independent origins and multiple material facets; require a direct identity witness for promotion, allow exact/alias registry resolution to warrant existing-Entity hydration or ambiguity, and retain a frequency-matched unresolved relation-only or mention-only twin; do not grow a semantic phrase blacklist.
...
- [Unsupported prose lowers recall] -> Keep `identity-frames-v1` closed and measurable; unresolved relation-only names remain below threshold until a direct identity witness appears, while exact/alias-resolved Entities remain eligible for relation-only hydration or ambiguity, and future corpus-derived warrants or frames require a versioned contract change rather than guessing.
