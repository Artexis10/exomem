...
- Added the encrypted `Retain`/`WaitForFirstConsumer` StorageClass, pinned Traefik and cloudflared, and rendered the three content-free scheduler CronJobs only from the byte-identical Substrate schedule contract (`02c1b9fa8d031ed77df9015f8555cfc95545b266767474aa52a8ccf08f4b40fc`).
- Split a cell release into a narrowly admitted root initialization Job and a normal restricted non-root StatefulSet. The serving pod has a read-only root, no API token, one 10 GiB retained PVC, a bounded 256 MiB temporary volume, fixed 5 GiB/90 MiB/zero-worker entitlements, one ClusterIP Service, quota/limits, and default-deny networking.
- K3s Pod Security defaults to baseline with restricted audit/warn. Only the named storage-init RuntimeClass bypasses restricted tenant enforcement, and fail-closed validating admission constrains that exemption to the immutable one-container init shape. A second tenant-cell policy rejects mutable images, host access, arbitrary service accounts, privilege, and foreign volume references in labelled tenant namespaces.
- Per-cell Traefik routing exposes only the Access-protected private v1 prefix and the two exact public v2 transfer paths, strips the opaque cell prefix, and has no NodePort or LoadBalancer Service. Routes remain disabled until the provisioner completes authenticated readiness.
- Helm strict lint passes for platform, serve, and initialize modes. Kubeconform 0.8.0 reports zero invalid resources across 50 platform, nine serving-cell, and eight initialization-cell resources. Ten Helm/Ansible contract tests, Ansible syntax/production lint, Ruff, ShellCheck, strict OpenSpec, and the complete 25-test hosted infrastructure suite pass.
- SOPS static-secret application, executable live admission/network denial probes, log-pressure alerting, and the real cluster/Cloudflare checks remain open; static rendering is not presented as live deployment proof.
