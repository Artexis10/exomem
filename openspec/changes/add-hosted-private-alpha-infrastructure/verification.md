...
- Split a cell release into a narrowly admitted root initialization Job and a normal restricted non-root StatefulSet. The serving pod has a read-only root, no API token or plaintext legacy credential environment variable, the complete hosted-v2 identity/runtime environment seam, one 10 GiB retained PVC, a bounded 256 MiB temporary volume, fixed 5 GiB/90 MiB/zero-worker entitlements, one ClusterIP Service, quota/limits, and default-deny networking.
