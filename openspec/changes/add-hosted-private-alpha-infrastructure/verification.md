...
- Helm strict lint passes for platform, serve, and initialize modes. Kubeconform 0.8.0 reports zero invalid resources across 50 platform, ten serving-cell, and nine initialization-cell resources. The complete pinned validation gate passes Terraform validation, Ansible syntax/production lint, Ruff, ShellCheck, strict OpenSpec, all 29 hosted infrastructure tests, and an ephemeral privileged container running exact K3s `v1.35.6+k3s1`; both live policies report no CEL warnings, routine contract mutations deny, an operator-authored legacy image annotation cannot admit a wrong image, and all positive/adversarial pod shapes above behave as specified.
