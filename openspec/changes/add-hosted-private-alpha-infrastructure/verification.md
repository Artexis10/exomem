...
- K3s Pod Security defaults to baseline with restricted audit/warn; every tenant namespace explicitly enforces/audits/warns at restricted v1.35, while only the named storage-init RuntimeClass bypasses Pod Security. One global platform-owned validating policy scopes itself to labelled tenant namespaces or any use of that exempt RuntimeClass and reads their non-secret exact cell contract annotations. It denies the exempt RuntimeClass outside a governed tenant namespace, admits the one immutable root init shape and restricted serving shape inside it, leaves ordinary non-tenant pods alone, and rejects a foreign image, capability, compute envelope, PVC, Secret, ConfigMap, unbounded temporary volume, sidecar, or init container. Namespaced ConfigMap parameters were tested first but exact K3s 1.35.6 returned stale/missing parameter data until an API-server restart, so that unsafe dynamic path was replaced by the namespace contract consumed directly by admission.
- Per-cell Traefik routing exposes only the Access-protected private `/v1` segment (exact path or descendants under `/v1/`) and the two exact public v2 transfer paths, strips the opaque cell prefix, and has no NodePort or LoadBalancer Service. The Terraform Tunnel target is cross-checked against the actual production Helm release Service `exomem-platform-traefik.exomem-platform.svc.cluster.local:80`. Routes remain disabled until the provisioner completes authenticated readiness.
- Helm strict lint passes for platform, serve, and initialize modes. Kubeconform 0.8.0 reports zero invalid resources across 48 platform, ten serving-cell, and nine initialization-cell resources. The complete pinned validation gate passes Terraform validation, Ansible syntax/production lint, Ruff, ShellCheck, strict OpenSpec, all 28 hosted infrastructure tests, and an ephemeral privileged container running exact K3s `v1.35.6+k3s1`; the live API reports no CEL warnings and admits/denies the positive and adversarial shapes above.
