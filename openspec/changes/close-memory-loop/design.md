...
Sources are append-only in their body, and a frontmatter status change is their established lifecycle mutation, so a recap is never rewritten in place. A revision is a new page, and the same writer batch marks the episode's previous live revision `superseded`, compare-and-swapped against that page's content hash. A concurrent revision fails the batch, which is retried once from a fresh read. One episode therefore has at most one live recap. The recap filename carries a short digest of the episode key and a microsecond recording-order token, so the revisions of one episode are found from one listing of one folder and ordered without reading every page, and without trusting modification times that a supersession mark also bumps.
