...
The first recovery integration is a read-only internal receipt adapter. It takes an uncertain leaf from owner-held episode state, reloads the exact sealed curation plan and approval, validates its run/plan/fingerprint/ordinal/step/operation/effect binding, reconstructs authoritative receipts, and verifies the atomic witness against current postimages before creating a committed observation for the pure model. Hold the existing vault consistency guard throughout this read so canonical writers cannot produce a mixed-time multi-path observation; this reuses the mutation boundary without requiring writer authority. It accepts no caller-supplied success proof, executes no writer and repairs no receipt. A witness without its terminal receipt remains uncertain until the existing curation owner recovers it. An as-yet unreconciled historical commit whose live postimage changed also remains uncertain rather than acquiring a fabricated current readback digest; this blocks retry while preserving canonical evidence. A failed receipt or absent witness alone cannot prove noncommit for the current episode attempt, because its binding does not yet identify the corresponding curation attempt.

The internal storage slice retains accepted events beneath the existing curation persistence boundary. The journal holds a bounded original evidence reference/excerpt, finite transition intents, validated sealed-plan bindings and adapter-generated committed outcomes; no caller may submit a materialized state or success proof. Reconstruct historical state by replaying accepted events without consulting mutable live postimages. A successfully verified commitment remains historical even after later edits, and cannot become retryable. Fresh postcommit attestations must reverify all claimed effects against current curation evidence under the same consistency boundary; reads label current coverage unchecked. A revision/hash compare-and-swap prevents competing sessions from overwriting accepted history. Validate the bounded hash chain, shapes and pure state transitions on every read; its digest detects corruption, not a malicious vault owner's edits. Reuse the existing no-follow curation reads and guarded atomic writer, with no second executor or database. The owner must durably record an uncertain attempt before calling its existing authorized writer. Storage is internal only: current authorization and retention for all supplied input/intent are trusted-owner preconditions, not implemented public permission checks. Evidence reference resolution, disclosure/retention enforcement, attempt-specific noncommit proof and public command integration remain required unfinished work in 3.2/3.3.

#### Principal-bound input recovery

The input owner facade binds every call to the current resolved canonical audience, not a client brand, transport, conversation or authorization-session identifier. Its version-2 journal envelope includes that audience in the root seal and storage identity while retaining the pure model's logical episode identity. Store version-2 journals within an audience-hash directory so later bounded pending-work discovery need not scan other audiences' journals; this slice adds no listing surface. Different audiences may use the same logical episode key without sharing history. An unbound or unresolved caller fails closed; an ownerless version-1 journal is never adopted by its first reader. The internal version-1 store remains a trusted-only compatibility seam.

The first facade accepts a canonical page or exact semantic-unit reference and resolves it under the current principal before recording a server-observed version digest. It stores references and digests, not another copy of source text. Recovery repeats exact resolution, lifecycle, release and version checks against the same immutable page snapshot. Missing, ambiguous, withheld or superseded evidence is unavailable; changed evidence is stale. Neither state returns original text, alternative matches or current replacement content. Full-page input and exact-unit input have a finite response byte cap; truncation must not masquerade as complete recovery. Digest-only conversation input remains explicitly unrecoverable.

References within recovered prose must also respect current disclosure policy, including title, alias and normalized-name matches. Reuse the maintained name resolver and require current lookup evidence; an absent, stale or unresolved lookup cannot establish that a reference is safe. Do not rebuild a corpus index inside recovery. Checking a linked target's visibility must not record its unreturned body as disclosed; disclosure evidence must identify the representation actually returned.

Source/Evidence retention uses existing canonical writers and their current authority. The active agent may preserve original conversational material as a Source, or proof-bearing material as Evidence, then submit the returned stable reference for verification. A supplied excerpt is evidence of that excerpt only. The facade neither captures a transcript implicitly nor treats source readability, a caller hash, an old capture receipt or a `stored` flag as a retention or disclosure grant. Artifact-byte recovery requires a separately verified companion/artifact binding through the existing artifact owner; a companion page alone does not establish that its binary original was recovered.

Facade responses contain only bounded operational identifiers/revisions and an authorized input projection. Raw journals, sealed plans, arbitrary proposal/disposition text and other candidate payloads never enter that response. Reference inspection contributes to the existing disclosure collector; public command integration must use its normal successful-boundary receipt emission. This slice adds create/append-input/inspect/recover primitives only, with no public command or mutation executor. Current authorization of proposal targets and ordinary public transitions remains part of tasks 3.2/3.3 before exposure.

#### Episode recaps: retained input and the cross-client carrier

A conversation that saved nothing else must still reach the next session, on any client. Its carrier is a bounded recap the active agent authors, retained as a canonical `episode` Source under `Sources/Episodes/` through the ordinary Source writer rather than in a side store. The vault is already the one store every client reads; the canonical page is partitioned by the same audience and release plane as every other page, so no second disclosure policy is needed; and the page stays readable and rebuildable without the ledger. The recap is closed and small: a subject, a one-line summary, at most five items each for what was worked on, decided and left open, at most three short verbatim user statements and an optional client label, with the rendered body capped at 4 KiB. Up to three existing memory references the recap concerns are visibility-filtered for the recorder and retained only in the recorder's own ledger, never on the page every audience may read. Oversize, credential-shaped or malformed input is refused before anything is written. The server validates and renders the recap; it never summarises a conversation.

`episode_memory` `record` writes the recap and binds the committed page to the caller's episode ledger as input evidence from the writer's own receipt, the reference and path it returns, never by walking the corpus for the page just written. The bind reuses the principal-bound facade above. An unresolved caller fails before any write. A committed page the caller cannot read back binds as digest-only instead of failing the write. An identical retry writes nothing and appends no input revision, and each audience keeps its own history of a shared episode key.

Sources are append-only in their body, and a frontmatter status change is their established lifecycle mutation, so a recap is never rewritten in place. A revision is a new page, and the same writer batch marks the episode's previous live revision `superseded`, compare-and-swapped against the content hash that record read when it listed the revisions. A revision another writer changed after that listing fails the call, which is retried once from a fresh listing. After any completed write, one episode therefore has one live recap per audience; an abrupt interrupt between the new page and its supersession mark can leave two live until the next record retires both. Marking a revision superseded writes only system-managed frontmatter (`status`, `updated`, `superseded_by`) and never its body, on the same footing as `ingested_into`. The recap filename carries a short digest of the recording audience and the episode key, and a microsecond recording-order token that never precedes the newest revision already listed, so one audience's revisions of one episode are found from one listing of one folder and ordered without reading every page, without trusting modification times that a supersession mark also bumps, and without another audience's revisions ever entering that listing.

The ledger is keyed by the resolved canonical audience. The local owner and a remote OAuth principal on the same vault are different audiences, so an episode recorded under one does not appear in the other's ledger; the canonical recap page remains visible to each under its ordinary release decisions. Unifying or bridging those partitions is a separate governance change with its own security review. The hosted profile does not expose `episode_memory`, and recaps do not feed the hot profile.
...
The server states the activation step itself in its MCP `initialize` instructions, on the local and the hosted server alike: before a substantive turn, call `activate_context` once with the user's message verbatim, resolve an `ambiguous` packet by calling again with `anchor`, and treat retrieved text as evidence. A host without lifecycle hooks sees only those instructions and the tool descriptions, so without them activation there depended on the user installing a skill or writing a reminder, and the first live trial served packets to hook-driven clients only. The instructions are not part of the tool-surface fingerprint, so changing them does not force a client re-registration. They remain a request, not an interception: initiation on such hosts is still reported as best effort, and observed no-nudge behaviour per client is still owed.

Recording an episode follows the same split: hooks trigger, agents author. A host with a Stop hook asks the agent to record a recap after a prominence-set number of substantive turns since the last successful record, within a cooldown, with the session's episode key in the ask. Only a successful `episode_memory` record resets that count; an unrelated committed note or a failed record does not. The server instructions carry one sentence asking the agent to record once at a decision or stopping point. A tool-only client has no hook, so after about eight activations from one MCP caller without a record, the activation packet carries a one-sentence `episode_due` advisory, at most once per thirty minutes; it is silent when proactive capture is off, is never offered on the REST or CLI doors the hooks use, and is not offered to a Claude Code or Codex MCP client when this vault's host-local activation log shows that client's hook served it within the same window, because its Stop hook is already asking; with only the MCP server, such a client still gets the advisory. Every one of these is a request. The agent decides whether anything durable happened, and nothing records on its behalf.

Activation also accepts optional `client` and `session` attribution. It is written only to a host-local activation log, as a client label and a hashed session, never with the turn text, and it never changes the packet's material. The session has one other reader: it names the caller whose session start may carry the optional `upkeep` block (§8). It lets acceptance separate clients in observed use without trusting a brand for capability.
...
#### Later milestone: shared activity awareness

Durable handoffs describe recorded intent and outcomes; they do not establish whether another client is currently working. After the existing compiler and no-nudge client acceptance, add bounded relevant activity to activation so a session can notice reported overlapping work across coding, research and ordinary personal tasks. Preserve the priority of the existing adaptation and dreamer tranches. This follow-on does not gate compiler delivery, candidate iteration or hosted launch, and this specification does not enable activity collection or communication.

Before implementation, audit current host/harness activity, session identity, dispatch records, handoffs and continuity carriers at their actual revisions. Reuse compatible mechanisms and name any missing cross-host transport, scope or freshness contract before adding a channel. The current `coordination_status` delegates to writer-lease status; it is storage-writer safety, not conversation/task ownership. Episode identity and continuity tokens likewise do not independently prove current activity. Keep native runtimes responsible for execution and messaging optional.

The minimum interoperable record carries a scoped activity identity, bounded subject/action, authenticated reporting principal with attributable client/session identity, report revision, reported status, service-observed contact time and authorized result/handoff references. Authentication identifies the reporter; a client claim is still reported activity, not verified progress. Replayed or out-of-order updates must not revive an expired or terminal report. Define finite retention, per-principal publication bounds and a bounded read projection in the implementation packet. No transcript, credential, arbitrary execution payload or speculative user commitment is required.

Activation must distinguish durable recalled context, freshly checked external evidence and reported client activity. Label each report with provenance, freshness and participation coverage; an unsupported client is unobserved, and an empty or unavailable feed does not mean nobody is working. A heartbeat proves contact only. Expiry means stale/unknown, not successful completion, release of exclusive ownership or permission to take over. A reported completion remains attributed until its result references are verified under the relevant task's evidence requirements. Conflicting advisory claims remain visible within budget rather than being silently resolved by timestamp or similarity.

Keep transient presence and advisory ownership in operational state with explicit expiry and retention, not canonical knowledge pages or graph edges. The episode compiler may preserve supported durable outcomes and handoff references through existing governed writers; presence is neither independent source evidence nor a completed episode. Both publication and reading must respect user, workspace and audience boundaries, including access changes and linked-result disclosure. Relevant activity shares the existing total activation token/latency bounds and has a finite sub-budget; report truncation/unavailability without hiding needed durable context. Capture and activation remain usable when activity is disabled or unavailable.

Advisory overlap can reduce repeat work but cannot guarantee exclusive action. It grants no permission to contact, stop or assume another session's task. Any later exclusive-claim contract requires a separately assessed atomic owner, expiry/recovery and execution-side fencing/idempotency at the actual side-effect owner. Do not repurpose the vault writer lease as a user-task lock or infer exclusivity from heartbeats. The first milestone is bounded awareness, not a scheduler or a second mutation executor.
...
Add a paired temporal-continuity acceptance journey: interrupt a topic, record a relevant subsequent development and an unrelated newer event, then resume from another supported session. The agent must connect the relevant development and older dependency, preserve unfinished work and apply the current correction without a retrieval reminder. A matched control re-edits an old note without a new event and must not manufacture freshness. Measure useful connections in the later response separately from timestamps present in a packet or forced tool calls.
...
#### Embeddings are the multilingual relevance sensor

Lexical contact reads the words a turn shares with an anchor. A German or Russian turn about an English-authored anchor therefore reaches it only through the anchor's invented name, and a Japanese turn written without spaces shares no token with anything. The anchor signature vectors are the sensor for that gap, under one rule: similarity orders and proposes, it never serves. `vector_band` is contact of the retrieved kind. It resolves an anchor only beside worded contact or valid continuity, never alone; it never carries a page, never vetoes one, and never decides what a referential turn refers to.

- **The band is corpus-relative.** For one turn the population is the anchor signature matrix. With `m` the median of the turn's similarities and `σ = 1.4826 · MAD`, an anchor bands when its similarity is at least `m + k·σ`, where `k = Φ⁻¹((1−α)^(1/N))` is the level the largest of `N` independent Gaussian similarities exceeds with probability `α` (0.01). That `α` is nominal, not the rate the band delivers. Measured, the share of turns with at least one band over unrelated anchors was 3-4% at `N` = 50-80 with independent Gaussian nulls and higher with heavy-tailed ones; with the served bge-m3 over 2,000 generated anchors it was 8.5-12.7% at `N` = 60-200 and about 6% at `N` = 1,000-2,000, about half of it topical adjacency; content-free turns banded 0 times in 310. A band alone therefore never resolves an anchor: it proposes, and worded contact or continuity decides. The rule is invariant to a constant offset (a turn in another language scores lower against everything) and to a scale (multilingual models compress cosines), so no threshold belongs to one model or one language. More than three anchors clearing, the rare-term anchor cap, is a topic rather than a referent, and none bands. Below 50 signatures, or with a degenerate spread, there is no chance level to measure against: the state is `uncalibrated` and the turn is not encoded.
- **One encoder, one identity.** Activation encodes with the server's resident model (`BAAI/bge-m3` on ONNX Runtime int8 with external data, on personal hosts), which is recall's own instance when the two profiles are equal. Its fingerprint names the model, the pinned revision, pooling, normalisation, the quantisation, the file format and the artefact digest. Signature vectors are stored under that fingerprint and compared only with a turn encoded under it, so a changed encoder reads `absent` until a background pass re-embeds. A turn is read at its first 40 tokens. The artefact is fetched from a published asset verified against the pinned digest, and built on the host only when that is unavailable; a mismatch is refused and rebuilt.
- **Incremental vectors, no request-thread load.** A catalogue change re-embeds only the anchors whose signature changed; a full re-embed happens only when the fingerprint changes. An index update on a request thread never loads a model: it embeds a bounded number of changed signatures with an encoder that is already resident and leaves the rest to the background pass. A request reads one cached, normalised matrix per index write.
- **CJK containment is weak worded contact.** In a script written without spaces (Han with kana, Thai, Lao, Khmer, Myanmar), an anchor name of at least two code points that lies wholly in the run's script, is contained in the run and is rare (named by at most three anchors) earns `rare_term`, never `exact_alias`. A name whose every occurrence sits inside a longer contained name is consumed by it. Like any rare term it resolves only beside a second contact such as the band, so a name inside an unrelated compound stays partial.
- **The recent block is not reordered.** A measured relevance rule for the recent-context menu (pool-only centroids) promoted a poison page under int8 single-text encoding, and with a guard it lifted too few gold pages to justify a ranking change. The block stays the recency block with semantic evidence on, off or degraded; relevance ordering waits for learned signals.
- **The band is the owner's under governance.** The band is an aggregate over the whole anchor catalogue: its population, floor, median, spread and width rule count every anchor, withheld ones included, so for a caller who may not see some anchor it would be a channel. Under a non-empty governed policy it runs only for owner-bound principals; every other principal gets `audience_restricted` and no `vector_band` contact, decided before any similarity is computed, and the packet cache keys on it. With an empty policy, or for the owner, nothing changes. The cost is that restricted callers on governed vaults fall back to lexical contact; none exist on personal hosts today.
- **Degradation is one shape.** Disabled, warming, busy, unavailable (encoder not resident), absent, uncalibrated and audience_restricted all serve the anchors, statuses, evidence and recent block that semantic-off serves; only the reported state differs.
- **Stated limits.** A rare word naming a page plus the band resolves that page even for a turn about something adjacent (converting a grill's target temperature resolves the grill page); the agent can discount it, and step-5 learning from agent picks is the corrective. An unspaced script is never carried (the lexical lane's rule). A minority-language function word that is rare in an English-majority vault can still pair in the carry. The hook's short non-English turns are not measured here. Hosted cells stay English until a shared per-node encoder exists.
...
The profile's sources are a heat projection, not file times (step 5). A machine-local ring of typed events is recorded by origin, where each act happens: a governed write made outside any batch scope is `work`, an agent's admitted pick is `pick`, and a page a recorded episode is about is `episode`, the three deliberate acts; a successful read and a citation by a governed write are selection; a captured session and an episode recap are contact only, read by the `recent_context` block and never a referent. A batch records nothing and a served packet records nothing, and the watcher's copies of the server's own commits are matched by signature and dropped, so timing — the write-burst rule — judges external edits only, and the cold seed, which has no history, is the one place the old cutoff after a burst survives. The referent comes from the latest working session only (a gap of six hours or more starts a new one), ranked by latest deliberate act, then latest selection, with no frequency term. "Decaying" means exactly that: a newer act displaces an older one, and a new session leaves the previous one behind; nothing expires on a clock. The heat is scoped to the caller's thread first: events carry the caller's opaque attribution — client, a session key and a workspace key, stored only as salted, audience-scoped derivations — and the referent and the `recent_context` order rank the caller's own session first, then its workspace, then the vault, a higher tier leading only while it holds a deliberate act or a thread the session was served. A single leading page that is not an anchor is carried on recency; a tie that mixes a page with anything else is reported, never guessed. The projection's output digest, as the caller ranks it, joins the packet cache key, a packet is compiled against the profile it was keyed on, and `generation.hot_profile` reports the profile's state (`current`, `partial`, `seeded`, `behind` or `empty`). This closes the three limits the recency prior shipped with: a maintenance batch erasing the user's last work, an edit made within a second of a batch being absorbed into it, and a read-only shift being served from the packet cache. One residual stays: an external edit within seconds of an external sync burst is still absorbed, because timing is all there is to judge it by, and the page regains heat with its next save.

The dreamer is bounded deterministic consolidation over changed material and indexed evidence, scheduled off the interactive path; its product noun is "upkeep". It is one background thread in the managed local service, default off, started after the graph drain and stopped with it; hosted cells do not run it. It reads derived state and pages and writes only its own disposable SQLite sidecar in the machine-local vault state directory. It never writes the vault, never takes the writer lease or mutation guard, never enqueues graph debt, never marks freshness pending, never builds or repairs an index and never loads or runs a model; each of those seven is pinned by a test. It runs only when the service is idle and the graph owes nothing, yields per page to any request, and has no maximum wait, because graph convergence strictly outranks it and nothing online waits for it. What changed comes from the freshness registry's delta or an exact diff of the live signature map against a persisted signature map, never from a filesystem walk or a whole-vault snapshot, so a write in the middle of a pass only queues that page again. Its six families (link, hydration, alias/anchor, convention/category, profile and episode-recap fold) are structural: counts of independent origins and graph shape, with no word list and no model. Independent origins come from a union-find over declared Sources, so one Source fanned out into many pages counts once; pages that declare no Source count as one origin between them, because the graph carries no session key that could tell their conversations apart. Each proposes one governed action through an existing writer and carries its evidence and versions; it never writes canonical knowledge autonomously. Its items use the `exomem://review/upkeep/<id>` namespace (link items reuse the relation queue's refs), and item, context and triage revalidate one proposal from its own pages without a whole-vault scan. An identity ambiguity is never proposed over: it is reported under the existing audit category that owns the defect, and quieting upkeep changes no other family or integrity surface. Pause, quiet, budgets, checkpointed continuation, deduplication and source-version invalidation are required, and online recall and capture remain usable with the dreamer disabled or failed.

Delivery rides the activation packet as an optional `upkeep` block with at most one item at a caller's session start: an explicit session key when the caller sends one, else the capture sweep's caller key with 30 minutes of quiet, held in memory. One vault receives at most one item per 10 minutes, one caller at most 3 per day, and one item at most two deliveries, the second a week later to another caller, after which it is held until its evidence changes. Those counts live in the sidecar's delivery ledger, written whether or not a pass runs (paused included) and kept across a row's eviction, within two bounds: a delivery whose row is gone is pruned 30 days after the newest delivery, and the ledger holds at most 2,048 rows, dropping the oldest orphaned deliveries first. The two directions of a link pair are one proposal: a review decision or a delivery hold on either direction holds the pair, and the row cap evicts both directions together. A stored direction is judged on its own id and current fingerprint. A direction whose row is gone is judged on any decision or delivery hold recorded against its id, whatever the fingerprint, until it is proposed again: its page is queued, and once it is proposed it is judged on its current fingerprint. The review state keeps a dismissal until it is reopened and a snooze until it lapses, so eviction alone never releases a dismissed or snoozed pair: it stays held while that decision stands and still binds the direction's current fingerprint; a pair held only by its deliveries is released when those ledger rows age out or are dropped, and can then be delivered at most twice more. A sidecar wipe (damage or a schema change) resets the ledger the same way, and that is an accepted limit: keeping it would need a second state file beside the one disposable sidecar, and every such re-delivery stays under the vault and caller caps. The item is counted in the packet's budget, served only when its evidence signatures are live and egress admits it, never truncated, never part of `due_state`, and skipped on a spent budget or any error; a failing worker shows a status line at a session start. Creating an ignored queue is not dreamer acceptance: verify next-session delivery and authorized disposition.

Frozen verifiers reuse the stricter canonical `frozen-verifiers` admission and effects contract: optional, default-off, version-pinned review labels with abstention. They cannot author canon, select entities, rank retrieval, gate capture or define policy. A verifier-admission evaluation is a programme task; enabling a model is not an acceptance prerequisite. Failure or resource pressure removes the label without changing online semantics. CPU-first evaluation precedes any separately evidenced GPU/co-tenant admission. The existing claim-polarity seam stays as it is, and upkeep does not use a verifier. That is reopened only when all four hold: a proposal family whose question is a pair stance exists; a fixture on that family's real pairs shows the label improves disposition at a false-positive rate no worse than its structural evidence; inference runs in a disposable child process within the resource envelope with no read regression in the write-burst probes; and the label is displayed only, never ordering or gating delivery.
...
the real page. A scheme'd reference is therefore never ambiguous: this one
split resolves it to exactly one vault path.

A PLAIN path-bearing field's string — `provenance.path`, `provenance.anchor`,
a `superseded_by`-style list entry — is not percent-encoded, so a literal `#`
or `|` in it cannot be told apart from a genuine trailing marker (`#current`,
`path.md#Heading`) by inspecting the string alone. The guard resolves this
under four rules:

- **R1 — never skip by default.** A non-empty string in a path-bearing field
  is always a candidate: decided, or withheld for being invalid. It is
  skipped outright only for an explicitly enumerated non-page shape (a
  memory-id reference, the synthetic `project:<key>` anchor id, the
  `plan:<rel>#<title>` anchor id) — never guessed at from how the string
  looks. An earlier version classified a candidate by shape (case-sensitive
  `.endswith(".md")`, a naive first-`#` split) before deciding whether to
  decide it at all; `secret#page.md` and `Secret.MD` were misclassified as
  not page-shaped and served in full, unfiltered, because nothing ever
  decided them.
- **R2 — one markdown predicate.** Whether a path names a markdown page is
  answered by exactly one case-insensitive function, reused everywhere this
  question is asked (the release decision itself, and the candidate/
  interpretation logic below) — never restated, and never case-sensitively.
- **R3 — interpretation sets for an ambiguous plain string.** A scheme'd
  reference has exactly one interpretation, per the split above. A plain
  string containing `#`/`|` is genuinely ambiguous between "a filename
  containing that character" and "a path plus a fragment/alias" — nothing in
  an unencoded string supplies the signal the scheme'd form's encoding does.
  The guard computes every plausible reading — the literal string, and every
  prefix ending exactly where a `#`/`|` immediately follows a markdown
  suffix — validates each as a safe in-vault relative path, and decides every
  reading that exists on disk. An earlier fix guessed one reading by cutting
  at the first `.md`: a withheld file literally named `notes.md#draft.md`
  was decided (and served) as its differently-permitted sibling `notes.md`
  instead of itself — the wrong file, decided with confidence. The item is
  served only when at least one reading exists and every existing reading is
  admitted; a reading that is merely syntactically safe but exists nowhere
  on disk (a phantom alternate reading of an ambiguous string) is never
  added to the withheld set on its own — doing so let a candidate's own
  literal text collide with itself in the canonical-key comparison every
  other field is checked against, withholding an ordinary unit under a
  policy scoped to an entirely unrelated folder.
- **R4 — key parity.** The withheld-key comparison expands to the canonical
  keys of every interpretation a withheld candidate's readings produced —
  a consequence of R3's architecture, not a separate mechanism: because both
  the plain and the URI form of the same reference ultimately decide the
  same real interpretation paths, a page withheld through one form is
  withheld through the other, and a page admitted through one is admitted
  through the other. The comparison this feeds — whether a value NAMES a
  withheld page, checked far more broadly than the candidate/decision path
  above (prose, titles, provenance, every consumer of the shared
  withheld-key matcher) — takes the OPPOSITE direction from a decision: a
  decision needs every EXISTING reading admitted to serve, but recognising
  that a value names a withheld page needs only ONE of its readings to
  match, since the value is being compared against an already-known
  withheld set rather than decided against the filesystem. Removing all
  positional splitting from a plain string's unwrap (R3) without also
  teaching this comparison to check every reading broke it silently: a
  provenance/title/prose field spelled with a genuine trailing marker
  (`path.md#Heading`) stopped matching its own withheld page at all.

"Path-bearing field" is scoped by field NAME, never by string shape: the
named scalar fields, authored prose's wikilinks, and a known reference-list
field's entries (`superseded_by` and its like — the same fields hit
projection's own withheld-provenance filter treats as path lists). An
ordinary scalar or list value under any OTHER key — `kind`, `status`,
`role`, `reason`, `lifecycle`, an `evidence` tag list — is never a candidate;
treating every string reachable anywhere in the packet as one turned an
ordinary tag value into a bogus "unresolvable" candidate whenever it did not
happen to exist as a file, which then wrongly withheld an unrelated
pointer/anchor/current-state item whose own field happened to share that
exact word.

A wikilink extracted from authored prose (`[[target|label]]`,
`[[target#Section]]`) is regex-captured WITHOUT its enclosing brackets before
it ever reaches the unwrap step, so the alias/heading split must be told
this text is already a wikilink target rather than detecting it from
surrounding brackets that will never be there; omitting that signal silently
stopped stripping the alias/heading off an extracted target, so a withheld
page named only through such a link in otherwise permitted prose was served
with the link intact. A wikilink target that unwraps to the empty string
(`[[#Heading]]` alone, naming no page) is inert in both directions: it
canonicalises to nothing, so it can neither match a withheld page nor become
a path candidate of its own; the unit it appears in is guarded normally by
every one of its other fields.
...
recognise, an empty path, or a plain string with zero safety-valid
interpretations — never becomes decidable, and is never handed to the
filesystem to find out. It is also never silently dropped: dropping it
...
project-anchor identifier, the synthetic plan-anchor identifier — is
exempted from this and stays skipped exactly as before; unwrapping never
...
Cross-client activity is retained as the later task-10 follow-on, after the existing tranches rather than ahead of compiler work. Its acceptance covers advisory awareness only; any exclusive-action implementation needs its own reviewed contract. Delivery of earlier tranches and hosted launch does not depend on enabling this follow-on.
