...
Activation corrections are learned from the agent's pick (step 5). When the user corrects which page they meant, the agent calls again with `anchor` set to it; at that seam, after the guard admitted the choice and before the pick is recorded as heat, the server classifies the pick from structure it already holds — `naming` (an anchor the turn's words did not reach, by the resolver's own rule over that one row), `cue` (a nameless, non-referential turn that picked a leading or recent page) or `referent` (heat only) — and counts a miss by path and class beside the heat ring. No word of the turn is persisted anywhere. A counted miss may carry one bounded advisory, never cached and never keyed, that names only the existing writers and their hash guards: `edit_memory` adding to the page's `learned_aliases`, and `schema_memory save-conventions` adding a referential cue to the vault's activation conventions; its `turn_terms` are the caller's own words, returned to that caller only, and never a word that already names an anchor. A learned name joins the anchor's activation aliases only, never its link or egress names or its identity; a learned cue changes only turn analysis, through its own digest in the packet cache key. The advisory writes nothing and grants nothing: "authored by the owner" includes the owner's agent's reasoned, hash-guarded governed save, never a server component acting on its own. Governed registry saves keep a log entry and a bounded snapshot, and `history` and `restore` make every change reversible.
