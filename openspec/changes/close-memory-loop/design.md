...
Activation also accepts optional `client` and `session` attribution. It is written only to a host-local activation log, as a client label and a hashed session, never with the turn text, and it never changes the packet. It lets acceptance separate clients in observed use without trusting a brand for capability.
