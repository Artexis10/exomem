...
Every agent mutation SHALL require a concise reason and SHALL return a bounded receipt containing collection identity, item identity, operation, before and after item/container hashes, affected canonical paths, and committed outcome. The existing human-readable activity log SHALL record agent mutations using the fields `operation`, `collection_id`, `item_key`, `before_item_hash`, `after_item_hash`, `before_container_hash`, `after_container_hash`, rationale, and one content-free mutation correlation marker without copying canonical item values. New agent-authored canonical blocks/items SHALL carry that same marker, so unmarked material is an explicit baseline while a marker without matching activity evidence is an audit gap. Direct human edits SHALL create detectable audit gaps rather than reconstructed events. Operational journals and governance receipts SHALL retain their existing distinct roles.
