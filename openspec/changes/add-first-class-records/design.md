...
The path stage treats every Records descendant as structured-only except exact `_collection.md` and exact lowercase `_summary.md` candidates. The content stage admits a summary only after a bounded descriptor-bound no-follow read of at most 65,536 raw bytes (including at most one BOM), UTF-8 decoding, a sibling matching Records manifest, and required frontmatter: `type: record-summary`, `summary_version: 1`, `derived: true`, matching collection UUID, lowercase source-snapshot hash, RFC 3339 generation time, and bounded Records query descriptor. Malformed, oversized, alias, symlinked, or mismatched summaries stay suppressed. A stale snapshot remains a clearly dated historical derived view and may remain discoverable without rescanning a thousand-item collection during indexing; inspection reports it stale, and no current-state operation treats it as current.
...
Generic `kb`/`vault` freshness remains for resolver/inbound/stable-reference consumers. Recall uses projected freshness/checkpoint views and every persistent semantic sidecar includes the recall-policy identity, so policy changes converge without file edits and raw-only edits do not churn recall caches. Background indexing applies local access-tier exclusion before reads; audience-specific release remains an egress responsibility because shared sidecars have no request principal. Collection-aware stable-reference and structured-query access remains available. Raw datasets already remain outside embeddings. A manifest-less legacy tracker remains available to explicit Records discovery/inspection even when it is not an ordinary semantic candidate.
