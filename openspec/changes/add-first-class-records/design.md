...
Markdown-log container hashes are SHA-256 over exact source bytes. Markdown-items container hashes are a bounded canonical snapshot of the marked manifest and the complete ordered source inventory `(path, kind, exact-byte hash)`; absent and empty sources differ and unexpected candidates are visible. Create is an `absent -> created` transition with null item key and meaningful marked-manifest/scaffold hashes. Existing writes publish canonical source first, manifest head second, then history; create uses an order which leaves the new manifest state inspectable at each prefix. Caught ordinary failures roll canonical/head/history back together; `BaseException` is never normalized and may leave a reportable partial prefix.

Inspection is report-only and reconstructs the predecessor chain from `record_audit.head`, independent of archive filename or enumeration order. Exact duplicate events dedupe; conflicting ID reuse, forks, wrong collection/path/item evidence, missing parents, discontinuity, old-head rollback, unmatched markers, and current-hash mismatch are gaps. It reports `baseline` only for an unmarked/headless collection after a complete bounded history scan finds no related event; `ok` only for a coherent chain to `baseline` or `absent`; `gap` for a positive mismatch; and `history_incomplete` when caps, unsafe/missing history, parsing failure, or drift prevents proof. Archive reads are bounded, descriptor-bound regular-file reads under the expected root and never follow symlinks, devices, or FIFOs. Canonical Records remain truth; inspection never repairs or invents history. Restoring both canonical bytes and audit history remains the explicit exact-byte ABA limitation.
