...
- Post-PR CI correction under the exact Python 3.11 lean shape: 8,006 passed, 132 expected environment/platform skips, and zero failures in 757.34 seconds. The recreated 3.11 environment omitted the optional TUI extra; the separate unchanged TUI gate remains 159 passed with all 26 snapshots. A Python 3.11-only pytest crash was traced to a Windows-dispatch test mutating process-global `os.name`; the test now uses a narrow module dispatch seam and passes both alone and in full-suite order.
