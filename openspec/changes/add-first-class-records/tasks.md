...
- Focused integration after final checkpoint/publication corrections: 183 passed. The independent adversarial review reran 131 exact publication, title-collision, addition/deletion, non-KB target, checkpoint-less, and reentrant graph checks with no findings. The earlier consolidated Records/concurrency/governance safety run passed 429 tests with 1 environment skip.
- Repository lean suite with embeddings and media extraction disabled: 7,261 passed, 83 environment/platform skips, zero failures in 759.15 seconds. The suite retains known repository warning output, including optional-dependency deprecations and the existing mocked media-supervisor thread warning.
...
- Packaged product: `scripts/e2e_product_loop.py` passed wheel install, governed stdio lifecycle/restart, authenticated HTTP/Studio, and two-replica writer coordination in 20.1 seconds. Wheel-only time-to-value passed in 4.3 seconds, including `exomem demo --json` and unattended setup.
- Static/public artifacts: targeted Ruff passed; CI-shaped mypy passed 13 source modules; `git diff --check`, capability generation, lock validation, and the 2,045-file public repository privacy scan passed.
- Scale/latency: latency gate passed 3 tests. The post-review semantic write gate at 2,000 pages measured validation median/p95 11.9/12.5 ms and commit median/p95 88.5/90.1 ms; at 8,000 pages, 51.6/118.8 ms and 286.1/344.1 ms. Cold graph activation remained outside measured samples.
- Independent review found and drove corrections for hidden identity/title collisions, authorization-before-resolution, bounded link candidate work, disclosure receipts, undeclared manual fields, Planning-profile refusal, tracker claim scope, exact graph and resolver checkpoint advancement, direct delete publication ordering, complete watcher publication retry, publication-failure cache withdrawal, resolver-topology reresolution, checkpoint-less full rebuild, and closing graph read snapshots before reentrant maintenance. The final post-fix review passed with no findings.
