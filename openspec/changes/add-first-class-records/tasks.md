...
- Repository lean suite on the final tree with embeddings and media extraction disabled: 7,666 passed, 96 environment/platform skips, zero failures in 648.74 seconds. The suite retains known repository warning output, including optional-dependency deprecations and the existing mocked media-supervisor thread warning.
...
- Packaged product: `scripts/e2e_product_loop.py` passed wheel install, governed stdio lifecycle/restart, authenticated HTTP/Studio, and two-replica writer coordination in 18.7 seconds. Wheel-only time-to-value passed in 4.5 seconds, including `exomem demo --json` and unattended setup.
- Static/public artifacts: both required Ruff gates passed; the CI-shaped mypy gate passed 7 source modules; `git diff --check`, capability generation, lock validation, the 2,095-file/2,111-text-payload public repository privacy scan, both distributions, ten skill archives, and the 15-file generated-artifact scan passed.
- Scale/latency: latency gate passed 3 tests. The post-review semantic write gate at 2,000 pages measured validation median/p95 14.6/19.0 ms and commit median/p95 109.5/113.0 ms; at 8,000 pages, 52.1/58.2 ms and 355.7/390.4 ms. Cold graph activation remained outside measured samples.
