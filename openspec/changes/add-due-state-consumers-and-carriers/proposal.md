...
- Complete the due-state consumer set: `prediction_window` (overdue predictions, unit-local predicate over `check_by` and `verdict`) and `unfinished_experiments` (elapsed past declared duration with no outcome) already shipped under `add-prediction-window-review` and `close-experiment-lifecycle` and are consumed here unchanged; this change adds the remaining two as audit categories with attention wiring and standard fingerprint triage — aging unanswered questions (`question_aging`, registered but not default, because its age threshold is system-invented) and supersession-integrity defects (`supersession_integrity`, dangling pointers and multi-headed chains, in the default union because the pointer is authored).
...
- `attention-queue`: two new signal categories (`question_aging`, `supersession_integrity`) with the standard review-item, fingerprint, and triage semantics, plus the maintained due-state projection contract with its invalidation, time-bucket, audience, and recovery rules. The two categories that `add-prediction-window-review` and `close-experiment-lifecycle` already own are consumed, not restated.
