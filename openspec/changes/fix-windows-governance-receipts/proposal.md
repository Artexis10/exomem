...
- Flush graph epoch directories when lifecycle abort removes newly staged floor/checkpoint artifacts, so a successful rollback preserves the original lifecycle refusal on Windows.
...
- Affected code: `src/exomem/governance/receipts.py`, `src/exomem/governance/lifecycle.py`, `src/exomem/graph_sync.py`, and shared secure filesystem helpers.
