...
- [x] 6.3 Harden the shared first-creation initializer for atomic-create races: only the winning creator applies a DACL, while losers use bounded validation-only stabilization and never repair an observed entry.
- [x] 6.4 Route both receipt-first and mutation-coordinator-first Windows lock initialization through that shared initializer while preserving the POSIX lock paths.
...
- [x] 7.3 Route graph epoch restoration's directory barrier through the shared Windows primitive and carry post-rename state into graph lifecycle abort; preserve the POSIX descriptor/fsync path.
- [x] 7.4 Run the masked lifecycle refusal regressions, post-rename failure regressions, and full governance receipt suite on native Windows.
