...
**Counts, not rows — and that excludes the aggregate.** Each executed evidence query contributes exactly `matched`, `returned`, `truncated`, and the collection/snapshot identifiers — never record rows or bodies. The first draft of this design also passed the saved view's declared `aggregate` through, which was wrong and was caught by mutation-testing review: the aggregate grammar admits `latest:<col>` (a full record row including `record_id` and `item_version`), `distinct:<col>` and `group:<col>` (record values), and `avg|sum|min|max:<col>` (a derived statistic, `avg` a float). A bound view may declare any of them, so the passthrough was simultaneously a row leak, an identity leak, and the score-shaped value this change forbids. It is dropped entirely. `total_matched` is computed identically under every aggregate shape, so the reader loses no count — and the refusal keeps a cross-profile review from becoming a bulk Records disclosure channel by another name.

**`returned` and `truncated` mean something different for an aggregating view.** For a plain view, `returned` is the number of rows in the page and `truncated` says the page was windowed. For a view that declares an aggregate, the projected envelope reports `returned: 0` — rows are suppressed in favour of the aggregate — and `truncated` reports that the *aggregate itself* was size-capped, not that rows were windowed. Nothing leaks either way and `matched` is unaffected, but a reader comparing two bindings should not read `returned: 0` as "no records" when the view aggregates. This is inherited from the shipped Records query contract rather than introduced here, and it is recorded so nobody later "fixes" the zero.
...
**Deterministic order, no ranking.** Items are ordered by `(collection_id, plan_id)` and evidence within an item keeps its authored descriptor order. Ordering carries no meaning, which is the point: nothing in the response implies that the first item is the one to worry about. There is exactly one ordering implementation — `order_items`, called by `review` before truncation, so identity order also decides which items survive the item cap. The guarantee is asserted on the returned sequence rather than on the helper, because a re-sort introduced anywhere downstream would otherwise pass a helper-only test.
...
**Bounded, non-disclosing unavailability reasons.** An evidence binding that cannot be executed is reported as `collection_unavailable`, `profile_mismatch`, `view_unavailable`, `query_unavailable`, `result_withheld`, or `budget_exhausted`. Missing and withheld collections both produce `collection_unavailable`, matching the parity rule the Planning relation envelope already established, so the review cannot be used to probe for hidden collections. A whole Planning collection that fails to resolve or query increments `collections_unavailable` and discloses nothing further — that counter is the difference between "nothing diverged" and "we could not look", so it is asserted rather than merely emitted.

`profile_mismatch` deliberately does distinguish a readable non-Records collection from an absent one. That is not a leak and must not be "fixed": the reason is only reachable after `resolve_collection` has already released the manifest to this caller, so it discloses nothing the caller could not read directly, and collapsing it into `collection_unavailable` would hide a real authoring mistake — a plan pointing its evidence at another plan.
