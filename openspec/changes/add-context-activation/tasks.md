...
- [ ] 1.1 Red: `tests/test_working_set_index.py` — build from a fixture vault with
...
- [ ] 1.2 Implement the sidecar (`.working-set.sqlite`, meta row, generation token,
...
- [ ] 1.3 Red: recall isolation — `ask_memory` fixtures byte-identical with and without
      the index present (reuse the referents identity test shape).

## 2. Anchor resolution (src/exomem/working_set_resolve.py)

- [ ] 2.1 Red: `tests/test_working_set_resolve.py` — evidence kinds per source; the
...
- [ ] 2.2 Implement turn analysis (NFKC/casefold tokens, n-grams, cue lexicon, one
...
- [ ] 3.1 Red: `tests/test_context_roles.py` — shipped registry loads; override may
...
- [ ] 3.2 Author `src/exomem/_scaffold/_Schema/context-roles.yaml` (14 roles, lanes,
...
- [ ] 4.1 Red: `tests/test_working_set_packet.py` — packet shape; per-role caps;
...
- [ ] 4.2 Implement lanes over `_find_semantic_units` (category pushdown, neighbourhood
...
- [ ] 5.1 Red: `tests/test_working_set_egress.py` — reuse the referents withheld-page
      fixture: no field of the packet names a withheld page; dependent evidence is
      dropped; `purpose` never enters the cache key.
- [ ] 5.2 Implement `guard_working_set` beside `guard_referents`, `working_set_runtime`
...
- [ ] 5.3 Red: `tests/test_activate_context_surface.py` — three-door parity on one
...
- [ ] 5.4 Latency gate: add `CEIL_WORKING_SET_MS` and the 2k→8k scaling bound to
      `tests/test_latency_gate.py` using the model-free synthetic corpus.

## 6. Derived artifacts and docs

- [ ] 6.1 Regenerate `tests/fixtures/mcp_tool_schemas.json` (`scripts/dump-tool-schemas.py`),
