...
- [ ] 1.1 Red: `tests/test_context_activation_fixtures.py` — the fixture manifest
...
- [ ] 1.2 Author the fixture manifest (C1–C9, T1–T9) and the seeded synthetic corpus
...
- [ ] 1.3 Author the private snapshot builder (local only): digest-pinned copy of a
...
- [ ] 2.1 Red: `tests/test_context_activation_audit.py` — per-case × anchor-kind
...
- [ ] 2.2 Implement the scorer over packets (`activate_context` output or an oracle
      packet file) and the report writer (per case, per class, duals, no aggregate).

## 3. Agent arms under `f32`

- [ ] 3.1 Red: existing `f32` variants byte-identical (seeds, oracles, paired
...
- [ ] 3.2 Implement the context-activation variants and arms (A1 control, A2 raw, A3
...
- [ ] 3.3 Implement the reminder-turn test, the blind extraction prompt with the
      model-free gold/poison intersection, and the blind rubric input; document the
      grader blinding.

## 4. Baseline runbook and measurements

- [ ] 4.1 Write `docs/benchmarks/context-activation.md`: order of measurement
