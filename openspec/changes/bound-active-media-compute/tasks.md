...
- [ ] 3.1 Add red selection tests for quiet/normal/performance workers using an ASR-specific CTranslate2/headroom probe, CUDA `auto` computation, CPU `int8`, and explicit overrides; cover CTranslate2 healthy with torch absent and torch healthy with CTranslate2 broken.
- [ ] 3.2 Add red installation/subprocess tests for direct `ctranslate2>=4.6.3,<5`, compatible Linux/Windows CUDA 12 wheels, parent-supplied child loader paths, and an incompatible system `libcublas.so.12` earlier in the parent environment; verify torch CUDA alone cannot satisfy ASR readiness.
- [ ] 3.3 Add red lazy-execution, fault-point, and upgrade tests for CUDA/cuBLAS/cuDNN/driver failures during model construction and segment consumption; verify the current generic state, sidecar-first crash window, retained legacy `FAILED` row/sidecar, repeated queued model attempt, and artifact-repair advice violate the blocked contract.

## 4. Repair ASR Accelerator Routing and Runtime Ownership

- [ ] 4.1 Add ASR-specific CTranslate2/headroom admission and permit its healthy CUDA result in normal/performance disposable workers while retaining quiet-mode and absent/declined-accelerator bounded CPU behavior; verify the torch-independent mode/device matrix passes.
- [ ] 4.2 Replace forced CUDA `int8_float16` with CTranslate2 `auto`, retain CPU `int8`, add the disclosed computation-type override, and verify an `sm_120` capability fake never selects the excluded INT8 path.
- [ ] 4.3 Package compatible CUDA 12 cuBLAS/cuDNN/runtime wheels plus direct CTranslate2 bounds, construct the wheel-owned runtime environment in the parent, and pass it to media-child `Popen`; make the verifier launch the same preconfigured hidden probe subprocess, regenerate the lockfile, and verify shadow-library tests plus `uv lock --check` pass.
- [ ] 4.4 Introduce a typed compute-runtime extraction failure and a ledger-first crash-consistent blocked protocol; verify fault after ledger marking converges the sidecar without ASR, the source artifact is unchanged, explicit retry reuses the job, and status never recommends artifact repair.
- [ ] 4.5 Add bounded startup reconciliation for retained legacy accelerator failures and a worker-lifetime circuit breaker for subsequent queued ASR jobs; verify old failed rows/sidecars converge to blocked and one runtime failure prevents repeated model construction.
- [ ] 4.6 Update the explicit media GPU verifier to use product routing and perform actual tiny-model compute in its preconfigured child before success; verify CPU-only reporting stays soft and mocked CUDA enumeration without model execution cannot pass.
...
- [ ] 6.1 Run `openspec validate --all --strict`, the full lean test corpus, lock/install checks, and Ruff at the completion boundary; verify the final tree is green or document only demonstrably unrelated baseline red with failure-name evidence.
