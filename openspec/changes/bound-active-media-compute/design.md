...
CUDA ASR defaults to CTranslate2 `compute_type="auto"`; CPU remains `int8`. `EXOMEM_ASR_COMPUTE_TYPE` provides an explicit disclosed override. CTranslate2's runtime-specific supported-computation selection is preferable to Exomem maintaining a GPU-generation table, and it avoids forcing INT8 where the installed CTranslate2 version excludes it.
...
Alternatives considered: hardcoding `float16` fixes the observed Blackwell case but can reject older GPUs that CTranslate2 could handle; preserving `int8_float16` ignores the engine's own supported-type contract; automatic CPU fallback obscures the fault and can recreate host saturation.

### 4. The media extra owns the CUDA 12 runtime CTranslate2 consumes

Linux and Windows media installs include compatible CUDA 12 cuBLAS, cuDNN, and runtime wheels. The media extra directly requires `ctranslate2>=4.6.3,<5`, the first Blackwell-capable CUDA 12.8 line, instead of relying on faster-whisper's transitive lower bound. A dependency-light helper discovers the installed `nvidia/*/lib` or `nvidia/*/bin` directories in dependency order and constructs the child environment. The media parent passes that environment to `Popen`; on Linux `LD_LIBRARY_PATH` and on Windows `PATH` are therefore correct before the child interpreter and dynamic loader start. Windows may additionally register DLL directories after startup, but no Linux correctness claim depends on mutating `LD_LIBRARY_PATH` in-process.

The standalone verifier is a parent/child command: the parent constructs the same runtime environment and launches a hidden probe subprocess, and only that child imports CTranslate2/faster-whisper. The probe reports the resolved native library identities. Installer/loader tests place an incompatible fake system `libcublas.so.12` earlier in the parent's environment and prove that a fresh child resolves the wheel-owned path first.
...
- [CTranslate2 `auto` can change its selected type across runtime upgrades] → Report the resolved runtime policy and require real model compute in the release probe.
...
4. Start the service, allow startup to reclassify retained accelerator failures, process one representative audio artifact, and measure process-tree CPU, health/status latency, job/sidecar state, selected device/type/runtime, and worker exit.
5. Reduce or remove the workstation's temporary quota drop-ins only after the packaged limits and measurements prove host responsiveness. Roll back by restoring the previous release while retaining the stricter local drop-ins; durable blocked jobs remain retryable and source artifacts require no migration.
