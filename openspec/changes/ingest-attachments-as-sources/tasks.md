...
- [x] 6.2 Test that a Source captured as an attachment can be promoted, and that both its bytes and its page arrive in Evidence with the pointer still correct. An artifact and its page are one unit: whichever half a caller names, the move normalizes onto the page — the `.md` path the semantic write machinery is built for — and renames the bytes alongside it in the same transaction, rewriting the pointer. Rollback undoes both, so a failure cannot split an artifact across two trees.
