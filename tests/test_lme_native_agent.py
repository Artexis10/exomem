...
def test_explicit_messages_and_initial_tools_bypass_lme_role_prompt(tmp_path):
    backend = Backend([{"role": "assistant", "content": "Done."}])
    b = broker(backend)
    explicit = [
        {"role": "system", "content": "TASK-ONLY-SENTINEL"},
        {"role": "user", "content": "Do the task."},
    ]
    result = asyncio.run(run_agent_phase(
        b, phase="writer", out=tmp_path / "run", messages=explicit, initial_tools=["ask_memory"],
    ))
    assert result["status"] == "completed"
    assert backend.requests[0]["messages"] == explicit
    assert "session-end memory maintenance" not in str(backend.requests[0])
    assert "Follow the installed memory skill" not in str(backend.requests[0])


def test_initial_tools_must_subset_available(tmp_path):
    b = broker(Backend([]))
    with pytest.raises(ValueError):
        asyncio.run(run_agent_phase(
            b, phase="writer", out=tmp_path / "run",
            messages=[{"role": "user", "content": "hi"}], initial_tools=["not_a_real_tool"],
        ))


def test_messages_and_turn_are_mutually_exclusive(tmp_path):
    b = broker(Backend([]))
    with pytest.raises(ValueError):
        asyncio.run(run_agent_phase(
            b, phase="writer", turn="also here", out=tmp_path / "run",
            messages=[{"role": "user", "content": "hi"}],
        ))


def test_omitted_messages_and_tools_retain_old_lme_default_behavior(tmp_path):
    backend = Backend([tool("ask_memory", {"query": "current choice"}), {"role": "assistant", "content": "Cedar."}])
    b = broker(backend)
    result = asyncio.run(run_agent_phase(b, phase="answer", turn="What is the current choice?", out=tmp_path / "run"))
    assert result["status"] == "completed"
    assert result["answer"] == "Cedar."
    assert "Answer the current user's question" in backend.requests[0]["messages"][0]["content"]
