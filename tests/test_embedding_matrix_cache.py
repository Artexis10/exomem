...
def test_full_reload_decodes_every_vector_in_one_buffer(tmp_path, monkeypatch):
    """A cold reload is one decode of the whole matrix, not one array per row.

    The per-row `frombuffer` + `stack` was most of a cold reload's CPU at
    ~58k rows; the rows it produces are unchanged, bit for bit.
    """
    vault = _fresh_vault(tmp_path)
    seed = embeddings.EmbeddingIndex(vault)
    ...
    for name in ("c.md", "a.md", "b.md"):
        vectors = rng.standard_normal((4, embeddings.VECTOR_DIM)).astype(np.float32)
        seed.upsert_file(name, [f"{name} {i}" for i in range(4)], vectors, 1.0)
        expected.update({(name, i): vectors[i] for i in range(4)})
    ...
    monkeypatch.setattr(np, "frombuffer", counted)
    loaded = embeddings.EmbeddingIndex(vault)._load_all_rows()
    monkeypatch.setattr(np, "frombuffer", real)

    assert calls["n"] <= 1
    assert loaded.metadata == sorted(expected)
    assert loaded.matrix.dtype == np.float32
    assert loaded.matrix.shape == (12, embeddings.VECTOR_DIM)
    for row, key in enumerate(loaded.metadata):
        assert np.array_equal(loaded.matrix[row], expected[key])
    loaded.matrix[0, 0] = 1.0  # a reloaded matrix stays an ordinary writable array


def test_full_reload_refuses_a_vector_of_the_wrong_width(tmp_path):
    """One short blob must never shift every later row into the wrong vector."""
    vault = _fresh_vault(tmp_path)
    seed = embeddings.EmbeddingIndex(vault)
    seed.upsert_file("a.md", ["a"], _mat([1, 0]), 1.0)
    seed.upsert_file("b.md", ["b"], _mat([0, 1]), 1.0)
    conn = sqlite3.connect(seed.path)
    with conn:
        conn.execute(
            "UPDATE chunks SET vector = ? WHERE file_path = 'a.md'",
            (np.ones(3, dtype=np.float32).tobytes(),),
        )
    conn.close()

    with pytest.raises(ValueError):
        embeddings.EmbeddingIndex(vault)._load_all_rows()
