...
def _context(tmp_path: Path):
    ...
    return ProviderSessionContext(
        run_id="hybrid-test", session_id="case-1", namespace="hybrid-test-case-1",
        work_root=tmp_path / "work", evidence_root=tmp_path / "evidence",
    )


def _provider(monkeypatch, tmp_path: Path):
...
def test_hybrid_rag_is_byte_identical_with_fixture_embedder(monkeypatch, tmp_path: Path) -> None:
    ...
    first = provider.retrieve("When does the clockwork fox visit?", 3, RetrievalPurpose.SCORED_RETRIEVAL)
    second = provider.retrieve("When does the clockwork fox visit?", 3, RetrievalPurpose.SCORED_RETRIEVAL)
    assert first == second
    assert repr(first).encode() == repr(second).encode()


def test_setup_refuses_to_pretend_a_model_without_the_declared_seam(monkeypatch, tmp_path: Path) -> None:
    from lme.providers.hybrid_rag_direct import HybridRagDirectProvider
    ...
    with pytest.raises(RuntimeError, match="PROTOCOL_FIXTURE_EMBEDDER"):
        HybridRagDirectProvider().setup(None, _context(tmp_path))
...
def test_chunking_prefers_sentence_boundaries_and_never_spans_sessions(monkeypatch, tmp_path: Path) -> None:
...
def test_short_session_yields_exactly_one_chunk_holding_the_whole_text(monkeypatch, tmp_path: Path) -> None:
...
def test_bm25_ranking_matches_the_rank_bm25_reference_implementation(monkeypatch, tmp_path: Path) -> None:
...
def test_rrf_tie_break_is_deterministic_for_equal_scores(monkeypatch, tmp_path: Path) -> None:
    ...
    for _attempt in range(2):
        provider = HybridRagDirectProvider()
        provider.setup(None, _context(tmp_path / str(_attempt)))
        provider.ingest_case(
        ...
        orders.append([hit.hit_id for hit in provider.retrieve("harbour lantern", 3, RetrievalPurpose.SCORED_RETRIEVAL)])
    assert orders[0] == sorted(orders[0])
    assert orders[0] == orders[1]


def test_cached_index_keeps_five_hundred_chunks_and_ten_queries_within_the_bound(monkeypatch, tmp_path: Path) -> None:
    ...
    for index in range(10):
        provider.retrieve(f"tok{index * 13} tok{index * 7} sentence", 10, RetrievalPurpose.SCORED_RETRIEVAL)
...
def test_retrieve_refuses_a_negative_top_k(monkeypatch, tmp_path: Path) -> None:
    from lme.providers.base import RetrievalPurpose

    provider = _provider(monkeypatch, tmp_path)
    provider.ingest_case([_event("Anything at all.")], _handle())
    with pytest.raises(ValueError, match="top_k"):
        provider.retrieve("anything", -1, RetrievalPurpose.SCORED_RETRIEVAL)
