...
nothing it depends on had changed. The build is a pure function of the
projection file, the review-state file, the audience, the purpose, the day
and the clock, so one build is kept per key and invalidated by exactly those.
"""
...
def _spy(monkeypatch: pytest.MonkeyPatch, horizon: dt.datetime | None = None) -> list[dict]:
    calls: list[dict] = []
    ...
    def fake(vault_root, *, today, now, principal=None, purpose=None, payload=None):
        ...
        return [{"ref": f"exomem://memory/{len(calls)}", "kind": "open", "path": PAGE}], horizon
...
def test_a_changed_release_verdict_rebuilds_before_the_file_changes(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _persist(tmp_path)
    calls = _spy(monkeypatch)
