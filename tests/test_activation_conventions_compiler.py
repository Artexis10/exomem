...
def test_the_resolver_floor_of_two_holds_even_if_ranking_config_asks_for_one() -> None:
    ...
    analysis = working_set_resolve.analyze_turn("tell me about beta notes")
    config = RankingConfig(working_set_lexical_min_terms=1)
    candidates = working_set_resolve.candidates_for(analysis, (row,), config=config)
    # Only "beta" and "notes" are shared broad terms (2), and "alpha" is not in
    # the turn — with the floor held at 2 this must NOT resolve lexical_overlap
    # off a single shared term even though the config asked for 1.
    assert candidates
