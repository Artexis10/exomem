...
def test_filter_by_project_singular(vault: Path) -> None:
    # research-note with project: project-alpha
    hits = find_module.find(vault, query="", projects=["project-alpha"])
    assert any(h.path.endswith("engine-architecture.md") for h in hits)
    # The insight has projects: [project-alpha, project-beta] — should also match
    assert any("progressive-disclosure" in h.path for h in hits)


def test_filter_by_project_plural(vault: Path) -> None:
    hits = find_module.find(vault, query="", projects=["project-beta"])
    # Only the insight has projects: [project-alpha, project-beta]
    assert any("progressive-disclosure" in h.path for h in hits)
...
def test_hit_carries_scope_for_different_types(vault: Path) -> None:
    ...
    assert research_hits[0].scope == "project-alpha"
...
def test_scope_kb_only_stays_strict(vault: Path) -> None:
    """scope='kb-only' is the strict opt-out: never widens to sibling folders."""
    hits = find_module.find(
        vault, query="reference-marker-xyz", scope="kb-only"
    )
    assert hits == []


def test_scope_vault_reaches_curated_trees(vault: Path) -> None:
    """scope='vault' walks the full vault and surfaces sibling-folder content."""
    hits = find_module.find(
        vault, query="reference-marker-xyz", scope="vault"
    )
    assert len(hits) == 1
    assert hits[0].path == "Reference/sample-curated.md"
...
def test_scope_default_is_kb(vault: Path) -> None:
    ...
    hits_default = find_module.find(vault, query="reference-marker-xyz")
    hits_kb = find_module.find(
        vault, query="reference-marker-xyz", scope="kb"
    )
...
def _make_x3_tracker(vault: Path) -> str:
    ...
    Mirrors a real-world `Tracking/X3 Full Reps.md`: no frontmatter, mostly
