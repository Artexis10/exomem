...
async def run_agent_phase(
    broker: NativeBroker, *, phase: str, turn: str | None = None, out: Path,
    messages: list[dict] | None = None, initial_tools: list[str] | None = None,
) -> dict:
    """Run a fresh worker; only the parent can call models or product tools.

    ``messages``/``initial_tools`` are an explicit-task escape hatch for
    non-LME callers (e.g. the utility instrument): when given, they replace
    the LME writer/answer role prompt and its default tool subset outright.
    Omitting both retains the original LME replay behavior.
    """
    import tiktoken
    ...
    if initial_tools is not None:
        if not isinstance(initial_tools, list) or any(not isinstance(n, str) for n in initial_tools) or not set(initial_tools) <= available.keys():
            raise ValueError("initial_tools must name only tools available in this phase")
        active = sorted(set(initial_tools))
    else:
        active = sorted({"discover_tools", "read_file", "bootstrap", "ask_memory", "read_memory"} & available.keys())
    if messages is not None:
        if turn is not None:
            raise ValueError("messages and turn are mutually exclusive")
        packet_messages = messages
    else:
        if not isinstance(turn, str):
            raise ValueError("turn is required when messages is not provided")
        packet_messages = _messages(broker, phase, turn)
    packet = {"messages": packet_messages, "tool_names": active, "max_ipc_bytes": limits.max_ipc_bytes}
    ...
    result = {"phase": phase, "status": "incomplete", "model_calls": 0, "tool_calls": 0,
              ...
              # Instrumentation; None means unmeasured, never an invented zero.
              "outcome_class": "incomplete", "failed_model_calls": 0, "unknown_usage_calls": 0,
              "model_seconds": 0.0, "tool_seconds": 0.0, "startup_seconds": None,
              "peak_context_tokens": 0, "cumulative_context_tokens": 0,
    ...
    try:
        ...
        async with asyncio.timeout(seconds):
            ...
            while True:
                ...
                if kind == "done":
                    ...
                    result.update(status="completed", answer=request["answer"], outcome_class="completed")
                    break
                if kind == "model":
                    ...
                    result["peak_context_tokens"] = max(result["peak_context_tokens"], context_tokens)
                    ...
                    result["cumulative_context_tokens"] += context_tokens
                    inflight = {"kind": "model"}
                    call_started = time.monotonic()
                    try:
                        completion = await broker.backend.complete_messages(messages, tools=tools, max_tokens=limits.max_output_tokens)
                    except BaseException as exc:
                        # A failed call is still a charge: keep whatever the
                        # backend was able to account for before re-raising.
                        result["model_seconds"] += time.monotonic() - call_started
                        result["failed_model_calls"] += 1
                        _absorb_usage(result, exc)
                        raise
                    result["model_seconds"] += time.monotonic() - call_started
                    ...
                    for field in ("cache_read_tokens", "cache_write_tokens", "reasoning_tokens"):
                        reported = getattr(completion, field, None)
                        if reported is not None:
                            result[field] = (result[field] or 0) + reported
                ...
                trace_response = {"direction": "response", **response}
                raw = getattr(broker, "last_raw_tool_result", None) if kind == "tool" else None
                if raw is not None:
                    trace_response["raw_result"] = raw
                _write(out / "events.jsonl", trace_response, append=True)
                await send(response)
    except asyncio.CancelledError:
        result["reason"] = "phase cancelled"
        result["outcome_class"] = "infrastructure"
        ...
        result["outcome_class"] = _outcome_class(exc)
        broker.envelope.stopped = result["reason"]
    finally:
        if inflight:
...
def _outcome_class(exc: BaseException) -> str:
    """Authored category for a phase that ended on an exception.

    Reads the exception's own declared category rather than its message: the
    text may carry provider data, and a benchmark decision must not depend on
    it. A declared envelope or deadline is an ordinary task-budget loss; an
    invalid model answer on a settled call is an actor failure; anything else
    is treated as infrastructure until proven otherwise.
    """
    if isinstance(exc, (EnvelopeExhausted, TimeoutError)):
        return "declared_exhaustion"
    return "actor_output" if getattr(exc, "benchmark_failure_category", None) == "actor_output" else "infrastructure"


def _absorb_usage(result: dict, exc: BaseException) -> None:
    """Keep a failed call's accounted usage; count it unknown when absent."""
    known = False
    for field in ("input_tokens", "output_tokens", "cost_usd"):
