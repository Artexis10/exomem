...
from .models import DatasetIdentity, RunManifest
...
def start_manifest(run_dir: Path | str, *, run_id: str, dataset: DatasetIdentity | dict[str, Any], started_at: str, namespaces: dict[str, str] | None = None, pins: dict[str, str] | None = None) -> RunManifest:
    path = _path(run_dir)
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        raise ManifestError("manifest already exists")
    manifest = RunManifest(run_id=run_id, dataset=dataset, status="started", started_at=started_at, namespaces=namespaces or {}, pins=pins or {})
